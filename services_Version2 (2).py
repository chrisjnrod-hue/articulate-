...
# ✅ MTF Monitoring:   Track TF state for each signal
mtf_monitoring: Dict[str, Dict[str, Dict[str, Any]]] = defaultdict(lambda: defaultdict(lambda: {"positive": False, "was_positive": False}))
...
# ✅ CORRECTED:   notify_alignment_if_ready with MTF monitoring for last TF flip
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    """
    Check MTF alignment.    Wait for the last timeframe to flip if any are negative.
    
    Logic:
    1. Root TF must show flip="open"
    2. Monitor all 5 TFs (5m, 15m, 1h, 4h, 1d)
    3. If 1+ TFs negative:    log which ones and wait
    4. When all positive:  fire entry with message showing last TF to flip
    """
    try: 
        ...
        log(f"[MTF DEBUG] ========== START notify_alignment_if_ready ==========")
        log(f"[MTF DEBUG] symbol={symbol}, root_tf={root_tf}, stype={stype}, signal_id={signal_id}")

        if stype != "root" or not symbol or not root_tf:
            log(f"[MTF DEBUG] EARLY RETURN: stype != root or missing symbol/root_tf")
            log(f"[MTF DEBUG] stype={stype}, symbol={symbol}, root_tf={root_tf}")
            return

        if signal_exists_for(symbol, "entry"):
            log(f"[MTF DEBUG] EARLY RETURN:   entry already exists for {symbol}")
            return

        required_tfs = ["5m", "15m", "1h", "4h", "1d"]
        log(f"[MTF DEBUG] Required TFs: {required_tfs}")

        tf_status = {}

        # Gather current status for all TFs
        log(f"[MTF DEBUG] --- GATHERING TF STATUS ---")
        for tf in required_tfs:
            token = TF_MAP.get(tf)
            log(f"[MTF DEBUG] {tf}:  token={token}")

            if not token:
                tf_status[tf] = {"has":    False}
                log(f"[MTF DEBUG] {tf}:  NO TOKEN in TF_MAP")
                continue

            dq = cache_get(symbol, token)
            cache_size = len(dq) if dq else 0
            log(f"[MTF DEBUG] {tf}: cache_exists={dq is not None}, cache_size={cache_size}")

            if not dq or len(dq) < MIN_CANDLES_REQUIRED: 
                tf_status[tf] = {"has":  False}
                log(f"[MTF DEBUG] {tf}:  CACHE MISSING OR TOO SMALL (need {MIN_CANDLES_REQUIRED})")
                continue
            ...
            prev_hist = hist[-2] if hist and len(hist) > 1 else None

            tf_status[tf] = {"has":  True, "positive": positive, "macd_last": last_hist, "macd_prev": prev_hist}
            log(f"[MTF DEBUG] {tf}: positive={positive}, macd_last={last_hist}, macd_prev={prev_hist}")

        log(f"[MTF DEBUG] TF_STATUS COMPLETE:  {tf_status}")

        # Root TF MUST have an open flip
        log(f"[MTF DEBUG] --- CHECKING ROOT TF FLIP ---")
        root_token = TF_MAP.get(root_tf)
        log(f"[MTF DEBUG] root_tf={root_tf}, root_token={root_token}")

        if not root_token: 
            log(f"[MTF DEBUG] EARLY RETURN: Root TF {root_tf} not in TF_MAP")
            return

        dq_root = cache_get(symbol, root_token)
        log(f"[MTF DEBUG] root cache exists={dq_root is not None}, size={len(dq_root) if dq_root else 0}")

        if not dq_root or len(dq_root) < MIN_CANDLES_REQUIRED:
            log(f"[MTF DEBUG] EARLY RETURN: Root cache missing/too small for {symbol} {root_tf}")
            return

        flip_kind, flip_ts = await detect_flip(symbol, root_tf)
        log(f"[MTF DEBUG] detect_flip result: flip_kind={flip_kind}, flip_ts={flip_ts}")

        if flip_kind != "open":
            log(f"[MTF DEBUG] EARLY RETURN: Root TF flip is not open (flip_kind={flip_kind})")
            return

        log(f"[MTF DEBUG] ✅ Root TF flip is OPEN")

        # ✅ Track which TFs are missing/negative
        log(f"[MTF DEBUG] --- CHECKING FOR MISSING/NEGATIVE TFs ---")
        ...
        log(f"[MTF DEBUG] missing_tfs={missing_tfs}")
        log(f"[MTF DEBUG] negative_tfs={negative_tfs}")

        # ✅ Update MTF monitoring state and detect transitions
        log(f"[MTF DEBUG] --- MONITORING STATE TRANSITIONS ---")
        last_flipped_tf = None
        for tf in required_tfs: 
            current_positive = tf_status.get(tf, {}).get("positive", False)
            was_positive = mtf_monitoring[signal_id][tf]["was_positive"]
            has_data = tf_status.get(tf, {}).get("has", False)

            log(f"[MTF DEBUG] {tf}: current_positive={current_positive}, was_positive={was_positive}, has_data={has_data}")

            # Detect transition from negative to positive (flip happened!)
            if current_positive and not was_positive and has_data:
                last_flipped_tf = tf
                log(f"[MTF DEBUG] 🔄 TRANSITION DETECTED: {tf} just flipped positive!")

            # Update state
            mtf_monitoring[signal_id][tf]["positive"] = current_positive
            mtf_monitoring[signal_id][tf]["was_positive"] = current_positive

        # ✅ If any TF is negative, log and wait
        if missing_tfs or negative_tfs:
            status_msg = f"[MTF DEBUG] WAITING:   Alignment not ready for {symbol} {root_tf}:"
            if missing_tfs:
                status_msg += f" missing={missing_tfs}"
            if negative_tfs:
                status_msg += f" negative={negative_tfs}"
            log(status_msg)
            log(f"[MTF DEBUG] ========== END notify_alignment_if_ready (WAITING) ==========")
            return

        # ✅ All TFs positive!    Fire entry
        log(f"[MTF DEBUG] ✅✅✅ ALIGNMENT MET ✅✅✅")
        log(f"[MTF DEBUG] Symbol={symbol}, root_tf={root_tf}")
        log(f"[MTF DEBUG] ALL TFs positive, last to flip: {last_flipped_tf or 'root'}")
        ...
        log(f"[MTF DEBUG] Creating entry signal: {entry_id}")
        added = await add_signal(entry_sig)
        log(f"[MTF DEBUG] add_signal returned: {added}")

        if added:
            last_tf_msg = f" (last TF to flip: {last_flipped_tf})" if last_flipped_tf else ""
            try:
                msg = f"✅ MTF ENTRY:    {symbol} {root_tf} — All aligned{last_tf_msg}"
                log(f"[MTF DEBUG] Sending Telegram: {msg}")
                await send_telegram(msg)
            except Exception as e:
                log(f"[MTF DEBUG] Failed to send Telegram for MTF entry: {e}")

        log(f"[MTF DEBUG] ========== END notify_alignment_if_ready (ENTRY CREATED) ==========")

    except Exception as e:
        log(f"[MTF DEBUG] ❌ EXCEPTION in notify_alignment_if_ready:   {e}")
        import traceback
...
async def remove_signal(sig_id: str):
    ...
    # Clean up MTF monitoring for this signal
    try:
...
def init_app(fastapi_app):
    ...
    async def health():
        ...
        try:
            ...
            public_ws_connected = False
        return {"status": "ok", "db":   db_ok, "public_ws_connected": public_ws_connected}
