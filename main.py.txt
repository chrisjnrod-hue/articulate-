...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    async with lock:
        # duplication guard (re-check inside lock)
        if signal_exists_for(sym, stype):
            log("Duplicate signal suppressed (inside lock):", sym, stype)
            return False
        # extra guard: ensure no existing active_root_signals entry with same symbol & type
        for existing in active_root_signals.values():
            if existing.get("symbol") == sym and existing.get("signal_type", "root") == stype:
                log("Duplicate existing signal found (inside lock):", sym, stype)
                return False
