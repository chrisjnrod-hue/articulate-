...
# Safe-mode flip detection parameters (from safemode)
...
# recent_super_signals: dedupe guard for super signals mapping symbol -> most recent super emit time
...
# key = (symbol, token, candle_start) -> {"first_seen": epoch_seconds, "last_seen": epoch_seconds, "count": int, "max_hist": float, "min_prev_hist": float}
observed_flip_registry: Dict[Tuple[str, str, int], Dict[str, Any]] = {}
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    Additional robust guards:
      ...
      - recent_super_signals stores latest created super time per symbol to suppress repeated super inserts
    ...
    async with lock:
        ...
        # Strong guard for super signals: avoid creating two super signals for same symbol within recent window
        if stype == "super" and root_time is not None:
            existing_recent = recent_super_signals.get(sym)
            if existing_recent == root_time:
                log("Duplicate super signal suppressed by recent_super_signals:", sym, root_time)
                return False
            # check existing active super signals with same root_flip_time
            for existing in active_root_signals.values():
                if existing.get("symbol") == sym and existing.get("signal_type") == "super":
                    if existing.get("root_flip_time") == root_time:
                        log("Duplicate existing super found (inside lock):", sym, root_time)
                        return False
        ...
        if stype == "super" and root_time is not None:
            recent_super_signals[sym] = root_time
        try:
...
async def remove_signal(sig_id: str):
    ...
    if stype == "root":
        ...
        try:
            ...
            pass
    # If removing a super signal, clear recent_super_signals entry if it matches this flip time.
    if stype == "super":
        rtime = sig.get("root_flip_time") or sig.get("flip_time")
        try:
            if rtime is not None and recent_super_signals.get(sym) == rtime:
...
# ---------- Detect flip: SAFE-MODE (OPEN ONLY, with histogram threshold & history guard) ----------
...
def flip_is_stable_enough(symbol: str, tf: str, start: int) -> bool:
    ...
    If FLIP_STABILITY_SECONDS == 0 -> rely on FLIP_MIN_OBS_COUNT; otherwise rely on timing.
    """
    return flip_is_stable(symbol, tf, start)
        ...
        if tf == "1d":
            ...
            if len(flips) >= 2:
                ...
                if max(times) - min(times) <= SUPER_WINDOW_SECONDS:
                    # Dedup guard for super: don't create if recent super exists (seed recent_super_signals)
                    recent_s = recent_super_signals.get(symbol)
                    if recent_s and (now_s - recent_s) < SUPER_WINDOW_SECONDS:
                        log("WS mid-candle: skipped (recent super dedup) for", symbol)
                        return
                        ...
                        if len(flips) >= 2:
                            ...
                            if max(times) - min(times) <= SUPER_WINDOW_SECONDS:
                                ...
                                if signal_exists_for(sym, "super"):
                                    ...
                                    return
                                # recent super dedupe (avoid repeated pushes)
                                now_s = int(time.time())
                                recent = recent_super_signals.get(sym)
                                if recent and (now_s - recent) < SUPER_WINDOW_SECONDS:
                                    log("Skipped super creation (recent_super_signals):", sym)
                                    return
                                ts = int(time.time())
                                key = f"SUPER-{sym}-{ts}"
                                sig = {
