...
BYBIT_API_key = os.getenv("BYBIT_API_KEY")  # keep legacy var present if referenced
...
# ---------- Missing helpers added (intervals, locks, closed-check) ----------
def get_symbol_lock(symbol: str) -> asyncio.Lock:
    """
    Return per-symbol asyncio.Lock (create if missing). Used to serialize signal/trade operations for a symbol.
    """
...
def interval_seconds_from_token(token: str) -> int:
    """
    Convert Bybit token ("5","15","60","240","D") to seconds.
    """
    try:
        if token is None:
            return 0
...
def last_candle_is_closed(dq: Deque[Dict[str, Any]], token: str, safety_seconds: int = 3) -> bool:
    """
    Determine whether the last candle in deque is closed.
    Logic:
      - If candle has explicit 'end' and it's <= now -> closed
      - Else compute start + interval_seconds - safety_seconds <= now => closed
      - Otherwise consider in-progress
    """
    if not dq:
        return True
    last = list(dq)[-1]
    ...
    try:
        ...
        return True
    interval = interval_seconds_from_token(token)
    if interval <= 0:
        # can't determine interval; fall back to end field behaviour (already checked)
        return False
    # if now has passed almost entire interval (within safety_seconds), count as closed
    return (now_s - start_int) >= max(0, interval - int(safety_seconds))
...
async def detect_flip(symbol: str, tf: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    Additional strictness added:
      - ensure last candle is in-progress (not closed)
      - ensure we are within a tight "open window" after candle start to count as flip-on-open (prevents detecting later in the candle or on the next candle)
    """
    if tf not in TF_MAP:
        return None, None
    token = TF_MAP[tf]
    ...
    if not hist_closed or not hist_with_inprog:
        return None, None
    prev_closed_hist = hist_closed[-1] if hist_closed else None
    cur_inprog_hist = hist_with_inprog[-1] if hist_with_inprog else None
    last_start = arr[-1]["start"]
    # enforce a tight "open window" - only detect flip within the first few seconds after candle start
    interval_secs = interval_seconds_from_token(token)
    # window = max(3s, 10% of interval) limited to 30s
    open_window = min(30, max(3, int(interval_secs * 0.1))) if interval_secs > 0 else 10
    age = int(time.time()) - int(last_start)
    if age > open_window:
        # Too late in the candle: be strict and don't claim "flip-on-open"
        return None, None
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    Add a signal with multiple duplication guards:
      ...
      - DB-level check for existing persisted signal with same symbol/root_tf/flip_time/signal_type
    ...
    async with lock:
        ...
        if sid in active_root_signals:
            ...
            return False
        # DB-level guard (prevent cross-process dupes and races)
        try:
            if db and root_time is not None:
                try:
                    query = "SELECT COUNT(*) FROM root_signals WHERE symbol = ? AND root_tf = ? AND flip_time = ? AND signal_type = ?"
                    async with db.execute(query, (sym, sig.get("root_tf"), root_time, stype)) as cur:
                        row = await cur.fetchone()
                        if row and row[0] > 0:
                            log("Duplicate suppressed by DB existing record:", sym, sig.get("root_tf"), root_time, stype)
                            return False
                except Exception as e:
                    # non-fatal
                    log("DB check for duplicate failed (continuing):", e)
        except Exception:
            pass
...
async def startup():
    ...
    # PublicWebsocketManager may be defined elsewhere in the real project.
    try:
        ...
        # If PublicWebsocketManager isn't available in this snippet, ignore and continue.
        public_ws = None
        log("PublicWebsocketManager not available in this context - continuing without live WS.")
