...
# Prefer the generic instruments-info (no category/instType filters) so Bybit returns the broadest set.
INSTRUMENTS_ENDPOINTS = [
    "/v5/market/instruments-info",  # generic endpoint returns all instruments (no query params)
    "/v5/market/instruments-info?category=linear",  # fallback to linear
...
async def get_tradable_usdt_symbols() -> List[str]:
    ...
    Changes:
    - Uses the more generic instruments endpoints to retrieve the broadest set of pairs.
    - Honor PERPETUAL_USDT_ONLY: when True, only include USDT perpetual pairs; when False (default) include all pairs.
    """
    ...
    for ep in INSTRUMENTS_ENDPOINTS:
        ...
        for it in items:
            try:
                ...
                # If PERPETUAL_USDT_ONLY is enabled, require USDT quote and perpetual/linear inst type
                if PERPETUAL_USDT_ONLY:
                    if not (symbol.upper().endswith("USDT") or quote == "USDT"):
                        continue
                    # optional: ensure inst_type mentions PERPETUAL/LINEAR if available
                    if inst_type and "PERPETUAL" not in inst_type and "LINEAR" not in inst_type and "PERP" not in inst_type:
                        # skip if inst_type is present and does not indicate perpetual
                        continue
                else:
                    # PERPETUAL_USDT_ONLY is OFF: include all quotes (don't filter USDT-only)
                    pass
    ...
    log("get_tradable_usdt_symbols: Found", len(uniq), "symbols (PERPETUAL_USDT_ONLY=", PERF := PERPETUAL_USDT_ONLY, ")")
...
def last_candle_is_closed(dq: Deque[Dict[str, Any]], token: str, safety_seconds: int = 3) -> bool:
    """
    Heuristic: if deque has last candle with 'end' or the end < now - small safety window -> closed.
    """
    if not dq:
        return True
    last = list(dq)[-1]
    ...
    if end:
        # end might be seconds
        now_s = int(time.time())
        return now_s >= (int(end) + safety_seconds)
    # If no explicit end, use expected interval duration
    try:
        start = int(last.get("start"))
        expiry = start + interval_seconds_from_token(token)
        return int(time.time()) >= (expiry + safety_seconds)
    except Exception:
        return False
...
async def log_current_root_signals():
    ...
    for s in active_root_signals.values():
        log("Root:", s.get("id"), s.get("symbol"), s.get("root_tf"), s.get("status"))
...
async def startup():
    ...
    - Gracefully skip optional components if they are missing or fail to initialize.
    - Start background tasks using a safe helper so task creation errors don't crash startup.
    """
    ...
    try:
        ...
        # Re-raise so Render will show the full traceback if DB init is fatal
        raise

    # Initialize optional components with defensive guards.
    # PublicWebsocketManager may not be defined or may raise in certain environments; guard it.
    try:
        PWM = globals().get("PublicWebsocketManager")
        if PWM:
            try:
            ...
            log("PublicWebsocketManager not found in globals; skipping public WS initialization.")
    except Exception as e:
    ...
    # Helper to create background tasks defensively.
    ...
    # Start background tasks. If any of these functions are missing or throw on creation, the error will be logged and startup will continue.
    try:
        safe_create_task(root_scanner_loop("1h"))
    except Exception as e:
        log("Could not start root_scanner_loop(1h):", e)
    try:
        safe_create_task(root_scanner_loop("4h"))
    except Exception as e:
        log("Could not start root_scanner_loop(4h):", e)
    try:
        safe_create_task(super_signal_scanner_loop_5m())
    except Exception as e:
        log("Could not start super_signal_scanner_loop_5m:", e)
    try:
        safe_create_task(evaluate_signals_loop())
    except Exception as e:
        log("Could not start evaluate_signals_loop:", e)
    try:
        safe_create_task(expire_signals_loop())
    except Exception as e:
        log("Could not start expire_signals_loop:", e)
