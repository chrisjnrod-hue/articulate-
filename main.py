# main.py
# Bybit MACD multi-timeframe scanner — updated to robustly fetch USDT perpetuals and
...
async def get_tradable_usdt_symbols() -> List[str]:
    ...
    # try endpoints in order. For debugging, log the first successful response body excerpt if parsing fails.
    found = []
    ...
    for ep in INSTRUMENTS_ENDPOINTS:
        ...
        if not items:
            # try nested shapes for older v2 endpoints (/v2/public/symbols)
        ...
        for it in items:
            try:
                # extract many possible key names
                symbol = (it.get("symbol") or it.get("name") or it.get("code") or it.get("symbolName") or "").strip()
                # quote coin may be under quoteCoin, quoteAsset, quote_currency etc
                ...
                # Accept a symbol if it clearly ends with USDT OR its quote field is USDT
                if not symbol:
                    continue
                if symbol.upper().endswith("USDT") or quote == "USDT":
                    # check status: prefer explicit 'Trading' but accept missing/empty
                    if status and isinstance(status, str) and status not in ("trading", "listed", "normal", "active", "open"):
                        # skip symbols with explicit non-trading status
                        continue
                    # prefer PERPETUAL / PERP if instType present
                    if inst_type and ("PERPETUAL" not in inst_type and "PERP" not in inst_type and "LINEAR" not in inst_type and "FUTURES" not in inst_type):
                        # uncertain inst type; still include if it is USDT
                        pass
                    found.append(symbol)
            except Exception:
                continue
        ...
        if found:
            ...
            # keep last excerpt to help debugging
            last_resp_excerpt = body_excerpt
...
def check_macd_flip_recent(klines: List[Dict[str, Any]], lookback: int = 3) -> Optional[int]:
    ...
    if not hist:
        return None
    # find flips where previous <=0 and current >0 searching latest `lookback` candles
