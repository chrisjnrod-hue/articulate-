...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    async with lock:
        ...
        # Immediately schedule evaluation for the newly added root/super to confirm MTF alignment quickly
        try:
...
async def remove_signal(sig_id: str):
    ...
    if stype == "super":
        # clear recent_super_signals if matches
        comps = sig.get("components", []) or []
        if comps:
            # components may be list of tf or list of (tf,ts) depending on creator; best-effort clear
            try:
                # if components encoded as e.g. ['1h','4h'] we can't derive timestamps; keep recent_super_signals expire naturally
                pass
            except Exception:
                pass
            ...
            if len(flips) >= 2:
                ...
                if max_ts - min_ts <= SUPER_WINDOW_SECONDS:
                    ...
                    sig = {
                        ...
                        "priority": "super",
                        "signal_type": "super",
                        # store components as list of (tf,ts) for canonical identity
                        "components": [{"tf": tfc, "ts": tss} for tfc, tss in flips],
                    }
                    added = await add_signal(sig)
                    if added:
                        # record recent_super_signals to prevent duplicates within SUPER_WINDOW_SECONDS
...
async def evaluate_single_signal(sig: Dict[str, Any]):
    ...
    try:
        ...
        # Ensure cached candles for all TFs
