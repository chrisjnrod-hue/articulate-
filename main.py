...
# recent_super_signals canonical mapping to avoid duplicates: symbol -> (min_ts,max_ts)
recent_super_signals: Dict[str, Tuple[int,int]] = {}
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    async with lock:
        ...
        # If this was a root/super, attempt to pre-warm caches and schedule immediate evaluation
        try:
            if stype == "root":
                root_tf = sig.get("root_tf")
                # prewarm required TF caches in background (fire-and-forget) to make evaluation fast
                for tf in tf_list_for_root(root_tf):
                    asyncio.create_task(ensure_cached_candles(sym, tf, MIN_CANDLES_REQUIRED))
                # slight delay to allow quickly-started prewarm fetches to begin (keeps behaviour robust)
                asyncio.create_task(_delayed_evaluate(sym, sig, delay=0.6))
            elif stype == "super":
                # warm the usual TFs and evaluate soon
                for tf in ["1h", "4h", "1d", "15m", "5m"]:
                    asyncio.create_task(ensure_cached_candles(sym, tf, MIN_CANDLES_REQUIRED))
                asyncio.create_task(_delayed_evaluate(sym, sig, delay=0.3))
        except Exception:
            pass

        return True

async def _delayed_evaluate(symbol: str, sig: Dict[str, Any], delay: float = 0.5):
    try:
        await asyncio.sleep(delay)
        await evaluate_single_signal(sig)
    except Exception as e:
        log("_delayed_evaluate error:", e)
...
async def remove_signal(sig_id: str):
    ...
    if stype == "super":
        # No guaranteed mapping removal required; recent_super_signals will naturally expire by time window
        pass
...
async def detect_flip(symbol: str, tf: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    """
    token = TF_MAP[tf] if tf in TF_MAP else tf
...
async def process_inprogress_update(symbol: str, interval_token: str):
    ...
    try:
        ...
        # For 1d token we might contribute to super-signal creation: run light super check
        if tf == "1d":
            # Check whether a super condition exists (2+ of 1h/4h/1d flips within window). We'll run detect_flip for the other TFs.
            flips = []
            for check_tf in ("1h", "4h", "1d"):
                try:
                    fk, ts = await detect_flip(symbol, check_tf)
                    if fk == "open" and ts:
                        flips.append((check_tf, ts))
                except Exception:
                    continue
            if len(flips) >= 2:
                ...
                min_ts = min(times)
                max_ts = max(times)
                if max_ts - min_ts <= SUPER_WINDOW_SECONDS:
                    super_key = f"SUPER-{symbol}-{min_ts}-{max_ts}"
                    existing_recent = recent_super_signals.get(symbol)
                    if existing_recent and existing_recent == (min_ts, max_ts):
                        log("WS mid-candle: skipped super creation (recent_super dedup) for", symbol)
                        return
                    if signal_exists_for(symbol, "super"):
                        log("WS mid-candle: skipped super creation (already active) for", symbol)
                        return
                    ...
                    sig = {
                        ...
                        "symbol": symbol,
                        "root_tf": "multi",
                        "root_flip_time": ts_now,
                        "root_flip_price": None,
                        "created_at": now_ts_ms(),
                        "status": "watching",
                        "priority": "super",
                        "signal_type": "super",
                        "components": [{"tf": tfc, "ts": tss} for tfc, tss in flips],
                    }
                    added = await add_signal(sig)
                    if added:
                        recent_super_signals[symbol] = (min_ts, max_ts)
                        log("WS mid-candle: SUPER signal detected for", symbol, "components:", sig["components"])
            return
    except Exception as e:
...
async def root_scanner_loop(root_tf: str):
    ...
    This function is resilient and returns on unexpected exceptions to allow outer restart wrapper to restart it.
    """
    ...
    while True:
        try:
            ...
            # Do NOT prewarm all symbols (can be heavy). detect_flip will fetch caches on-demand.
            tasks = []
            ...
            # Launch detection tasks in bounded concurrency and wait for them to finish (timeout to avoid hang)
            ...
            if tasks:
                try:
                    ...
                    log("root_scanner_loop gather error:", e)

        except Exception as e:
            log("root_scanner_loop outer error:", e)
        # ensure we always sleep once per loop iteration
        await asyncio.sleep(SCAN_INTERVAL_SECONDS)

# ---------- Evaluate single signal (immediate & periodic) ----------
async def evaluate_single_signal(sig: Dict[str, Any]):
    """
    Evaluate a single root/super signal immediately and attempt to create an entry/trade.
    This function is also used by the periodic evaluate_signals_loop to avoid duplicated logic.
    Sends a clear MTF confirmation Telegram alert when alignment is detected.
    """
    try:
        stype = sig.get("signal_type", "root")
        ...
        if is_stablecoin_symbol(symbol) or symbol_too_new(symbol):
            # remove signal if no longer acceptable
            await remove_signal(sig["id"])
            return
        ...
        # Ensure cached candles for all TFs (try to prewarm in parallel)
        try:
            await asyncio.gather(*(ensure_cached_candles(symbol, tf, MIN_CANDLES_REQUIRED) for tf in required_tfs), return_exceptions=True)
        except Exception:
            pass
        ...
        # Root must be present and flipped (root TF only for root signals)
        if stype == "root":
        ...
        if stype == "root":
            ...
            other_tfs = [tf for tf in required_tfs if tf != root_tf] if root_tf else required_tfs
        ...
        # At this point we have alignment readiness. Send a clear MTF confirmation alert.
        try:
            await send_telegram(f"MTF confirmed for {symbol} root={root_tf} (ready for entry)")
        except Exception:
            pass
...
# ---------- Evaluate existing signals (root/super) for entry/trade (periodic) ----------
async def evaluate_signals_loop():
    """
    Periodically iterate active signals and call evaluate_single_signal on each.
    """
    while True:
        try:
            ...
            for sid in ids:
                ...
                if not sig:
                    continue
                # schedule evaluation (don't block the periodic loop)
                try:
        ...
        try:
                ...
                async with semaphore:
                    try:
                        ...
                        if len(flips) >= 2:
                            ...
                            max_ts = max(times)
                            if max_ts - min_ts <= SUPER_WINDOW_SECONDS:
                                # canonical super key to dedupe repeated creations
                                super_key = f"SUPER-{sym}-{min_ts}-{max_ts}"
                                existing_recent = recent_super_signals.get(sym)
                                if existing_recent and existing_recent == (min_ts, max_ts):
                                    log("Skipped super creation (recent_super dedup) for", sym)
                                    return
                                ...
                                added = await add_signal(sig)
                                if added:
                                    recent_super_signals[sym] = (min_ts, max_ts)
            ...
            if tasks:
                try:
                    ...
                    log("super_signal_scanner_loop_5m gather error:", e)
        except Exception as e:
...
def start_background_task(coro_func, *args, name: Optional[str] = None):
    """
    Wrap a background coroutine in a resilient runner that restarts it if it crashes.
    """
    async def _runner():
        while True:
            try:
                ...
                log(f"Background task {name or coro_func.__name__} crashed: {e}; restarting in 2s")
                await asyncio.sleep(2)
            # tight yield to event loop before restart to avoid busy loop
...
async def startup():
    ...
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID and (_TELEGRAM_WORKER_TASK is None or _TELEGRAM_WORKER_TASK.done()):
        try:
            ...
            pass
    # Start background loops with resilience wrapper so they are restarted if they crash
    start_background_task(root_scanner_loop, "1h", name="root_scanner_1h")
    start_background_task(root_scanner_loop, "4h", name="root_scanner_4h")
    start_background_task(super_signal_scanner_loop_5m, name="super_scanner_5m")
    start_background_task(evaluate_signals_loop, name="evaluate_signals_loop")
    start_background_task(expire_signals_loop, name="expire_signals_loop")
    ...
    start_background_task(periodic_root_logger, name="periodic_root_logger")
