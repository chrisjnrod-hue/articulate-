...
async def debug_check_symbol(symbol: str = Query(..., min_length=1), tf: str = Query("1h"), _auth=Depends(require_admin_auth)):
    """
    Enhanced debug endpoint: returns prev_hist, cur_hist, the previous-N slice used
    by FLIP_MIN_NEG_CANDLES and a clear 'diagnosis' field explaining why detect_flip
    would accept or reject the open flip right now.
    """
    if tf not in TF_MAP:
    ...
    dq = cache_get(symbol, token)
    if not dq or len(dq) < MIN_CANDLES_REQUIRED:
        return {"symbol": symbol, "tf": tf, "cached_klines": 0, "error": "no cached klines / insufficient candles", "diagnosis": "no_data"}
    ...
    # prepare the slice of previous hist bars used by FLIP_MIN_NEG_CANDLES
    slice_prev = None
    if hist and FLIP_MIN_NEG_CANDLES > 0 and len(hist) >= (1 + FLIP_MIN_NEG_CANDLES):
        start_idx = max(0, len(hist) - 1 - FLIP_MIN_NEG_CANDLES)
        slice_prev = hist[start_idx: len(hist) - 1]

    # basic checks and human-friendly diagnosis
    last = list(dq)[-1]
    last_start = last.get("start")
    closed = last_candle_is_closed(dq, token, safety_seconds=3)

    diagnosis = []
    # closed candle -> we ignore closed flips (open-only)
    if closed:
        diagnosis.append("last_candle_closed: true -> ignore (open-only flips)")
    # ensure hist present
    if prev_hist is None or cur_hist is None:
        diagnosis.append("macd_insufficient_history")
    else:
        # primary numeric crossing check
        if not (prev_hist <= 0.0 and cur_hist > FLIP_HIST_THRESHOLD):
            # explain which numeric rule failed
            if cur_hist <= FLIP_HIST_THRESHOLD:
                diagnosis.append(f"cur_hist_too_small: cur_hist={cur_hist} <= FLIP_HIST_THRESHOLD={FLIP_HIST_THRESHOLD}")
            if prev_hist > 0.0:
                diagnosis.append(f"no_crossing_prev_positive: prev_hist={prev_hist} > 0 (no <=0 -> >threshold crossing)")
        else:
            # crossing numeric condition passed; check negative-history requirement
            if FLIP_MIN_NEG_CANDLES > 0:
                if slice_prev is None:
                    diagnosis.append("prev_hist_slice_missing")
                else:
                    any_pos = any((h or 0.0) > 0.0 for h in slice_prev)
                    if any_pos:
                        diagnosis.append(f"prev_neg_requirement_failed: slice_prev has positive entries {slice_prev}")
                    else:
                        diagnosis.append("prev_neg_requirement_ok")
            else:
                diagnosis.append("prev_neg_requirement_disabled")

            # record observation logic (what the real detect_flip does)
            key = (symbol, token, last_start)
            rec = observed_flip_registry.get(key)
            if rec is None:
                obs_count = 0
            else:
                obs_count = rec.get("count", 0)

            if FLIP_STABILITY_SECONDS > 0:
                # time-based stability
                if rec is None:
                    diagnosis.append(f"stability_waiting: first_seen_not_recorded (need {FLIP_STABILITY_SECONDS}s)")
                else:
                    age = int(time.time()) - int(rec.get("first_seen", 0))
                    if age < FLIP_STABILITY_SECONDS:
                        diagnosis.append(f"stability_waiting: observed for {age}s (< {FLIP_STABILITY_SECONDS}s)")
                    else:
                        diagnosis.append("stability_ok_time")
            else:
                # count-based stability
                if obs_count < FLIP_MIN_OBS_COUNT:
                    diagnosis.append(f"stability_waiting_count: obs_count={obs_count} < FLIP_MIN_OBS_COUNT={FLIP_MIN_OBS_COUNT}")
                else:
                    diagnosis.append("stability_ok_count")

    # Also compute what detect_flip() would return now for convenience
    flip_kind, flip_ts = await detect_flip(symbol, tf)

    # Build return object
    registry_info = None
    if last_start:
        registry_info = observed_flip_registry.get((symbol, token, last_start))
    ...
    return {
        ...
        "prev_hist": prev_hist,
        "cur_hist": cur_hist,
        "prev_hist_slice": slice_prev,
        "flip_hist_threshold": FLIP_HIST_THRESHOLD,
        "flip_min_neg_candles": FLIP_MIN_NEG_CANDLES,
        "flip_min_obs_count": FLIP_MIN_OBS_COUNT,
        "flip_stability_seconds": FLIP_STABILITY_SECONDS,
        "observed_flip_registry": registry_info,
        "detect_flip_result": {"flip_kind": flip_kind, "flip_ts": flip_ts},
        "diagnosis": diagnosis,
    }
