...
async def persist_root_signal(sig: Dict[str, Any]):
    """
    Persist root signal defensively: use INSERT OR IGNORE to avoid DB errors if a duplicate slip through.
    ...
    try:
        ...
        for sym, t in max_flip_per_symbol.items():
            try:
                ...
                pass
        log("Loaded", len(rows), "persisted root signals from DB and seeded recent_root_signals")
    except Exception as e:
        ...
        while True:
            try:
                ...
                if active_root_signals:
                    await send_telegram("Root signals summary:\n" + "\n".join([f"{s.get('symbol')} {s.get('signal_type')}" for s in active_root_signals.values()][:50]))
            except Exception as e:
