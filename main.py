# main.py
# Bybit MACD multi-timeframe scanner — complete file with safe startup wrapper.
# Replace your existing main.py with this file and redeploy.
...
# Import user's external WS client (if present)
try:
...
# Websocket URLs (we will use the user's BybitWebSocketClient if available)
...
# Integrate external BybitWebSocketClient if available; else none
...
async def status(_auth=Depends(require_admin_auth)):
    ...
    return {
        ...
        "private_ws_connected": bool(private_ws and getattr(private_ws, "conn", None) and getattr(private_ws, "authenticated", False)) if private_ws else False,
    }
...
async def startup():
    """
    Safe startup wrapper: initialize DB and start background tasks.
    Any exception during startup is persisted to raw_ws_messages and logged,
    preventing an immediate process exit so we can inspect the error.
    """
    ...
    try:
        ...
        # Start public WS using BybitWebSocketClient (best-effort)
        try:
            if BybitWebSocketClient:
                public_ws = BybitWebSocketClient(category="linear")
                asyncio.create_task(public_ws.connect())
                log("Scheduled public_ws.connect()")
            else:
                log("BybitWebSocketClient not available in environment.")
                await persist_raw_ws("startup", "public_ws_init_error", "BybitWebSocketClient import failed")
        except Exception as e:
            log("Failed to instantiate/connect public_ws:", e)
            try:
                await persist_raw_ws("startup", "public_ws_init_error", str(e)[:MAX_RAW_WS_MSG_BYTES])
            except Exception:
                log("Failed to persist public_ws_init_error")

        # Start private ws only if trading enabled (best-effort)
        if TRADING_ENABLED and PRIVATE_WS_URL:
            try:
                private_ws = PrivateWebsocketManager(PRIVATE_WS_URL, BYBIT_API_KEY, BYBIT_API_SECRET)
                asyncio.create_task(private_ws.connect_and_auth())
                log("Scheduled private_ws.connect_and_auth()")
            except Exception as e:
                log("Failed to instantiate/connect private_ws:", e)
                try:
                    await persist_raw_ws("startup", "private_ws_init_error", str(e)[:MAX_RAW_WS_MSG_BYTES])
                except Exception:
                    log("Failed to persist private_ws_init_error")

        # helper to schedule tasks with internal exception capture so one failing task doesn't kill the process
        def _safe_task(coro):
            async def _wrapper():
                try:
                    await coro
                except Exception as ex:
                    log("Background task error:", ex)
                    try:
                        await persist_raw_ws("background", "task_error", str(ex)[:MAX_RAW_WS_MSG_BYTES])
                    except Exception:
                        log("Failed to persist background task error")
            return asyncio.create_task(_wrapper())

        # start scanner tasks in safe wrappers
        _safe_task(root_scanner_loop("1h"))
        _safe_task(root_scanner_loop("4h"))
        _safe_task(active_signal_watcher_loop())

        log("Background tasks started")
    except Exception as e:
        # catch any fatal startup error, persist it, and avoid re-raising so the process stays alive
        log("Startup failed with exception:", e)
        try:
            await persist_raw_ws("startup", "startup_error", str(e)[:MAX_RAW_WS_MSG_BYTES])
        except Exception:
            log("Failed to persist startup error")
        # keep the service running in degraded mode so you can fetch logs and debug endpoints
        return
