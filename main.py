                    ...
                    for symbol, interval_token, candle, is_closed in entries:
                        try:
                            if is_closed:
                                ...
                                log("Merged closed kline from WS into cache for", symbol, interval_token, "start", candle.get("start"))
                        except Exception:
                            continue
                except Exception:
                    continue
...
# ---------- Detect flip on last CLOSED candle (returns (flipped_bool, last_start_ts_or_None)) ----------
async def detect_flip_on_last_candle(symbol: str, tf: str) -> (bool, Optional[int]):
    """
    Strict detection: returns (True, last_candle_start) if the most recent candle is closed and
    MACD histogram flipped from <=0 to >0 on that last closed candle. Otherwise (False, None).
    """
    token = TF_MAP[tf]
    ...
    if not dq or len(dq) < MIN_CANDLES_REQUIRED:
        return False, None
    # ensure last candle is closed before using it for detection
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if symbol_too_new(symbol):
                    continue
                try:
                    flipped, last_start = await detect_flip_on_last_candle(symbol, root_tf)
                    if not flipped:
                        continue
                    ...
                    if exists_same:
                        continue
                    # another duplication guard: avoid multiple root signals for same symbol
                    if signal_exists_for(symbol, "root"):
                        # If existing root signal's flip time is different but still recent, skip adding another one
                        continue
                    ...
                    if added:
                        log("Root flip detected and added:", symbol, root_tf, last_start)
...
async def evaluate_signals_loop():
    ...
    Rules:
      ...
      - Root must have flipped on its most-recent closed candle (we ensured when creating)
      - Among other TFs at most one may be negative; that single TF (the "last-to-flip") must have flipped on its most-recent closed candle for the entry to occur.
      - All other TFs must be MACD-positive on their last closed candles.
      - When the conditions are met, create an 'entry' signal and place (or simulate) the trade.
    Duplication guards:
            ...
            for sid in ids:
                try:
                    ...
                    if stype == "root":
                        ...
                        # super: evaluate as root with root_tf stored; for super we treat required TFs as 5m,15m,1h,1d (consistent)
                    ...
                    for tf in required_tfs:
                        ...
                        if not dq or len(dq) < MIN_CANDLES_REQUIRED:
                            ...
                            continue
                        # require last closed candle to be used for decision
                        if not last_candle_is_closed(dq, token, safety_seconds=3):
                            tf_status[tf] = {"has": True, "closed": False}
                            continue
                        closes = candles_to_closes(dq)
                        positive = macd_positive_from_closes(closes)
                        # check flip on last closed candle
                        prev_flip = False
                        try:
                            hist = macd_hist(closes)
                            if hist and len(hist) >= 2:
                                prev = hist[-2] or 0
                                cur = hist[-1] or 0
                                prev_flip = (prev <= 0 and cur > 0)
                        except Exception:
                            prev_flip = False
                        last_ts = list(dq)[-1]["start"]
                        tf_status[tf] = {"has": True, "closed": True, "positive": positive, "flip_last": prev_flip, "last_ts": last_ts}
                    # Root must be present, closed, and flipped on last closed candle
                    ...
                    if not tf_status[root_tf].get("flip_last"):
                        continue
                    # Count negatives among other TFs
                    ...
                    if len(negatives) > 1:
                        continue
                    # If exactly 1 negative -> that tf must have flipped on its last closed candle
                    if len(negatives) == 1:
                        tf_to_flip = negatives[0]
                        st = tf_status.get(tf_to_flip, {})
                        if not st.get("has") or not st.get("closed"):
                            continue
                    ...
                    # Achieved entry readiness. Create entry signal and place/simulate trade.
                    # Duplication guard: skip if entry already active or open trade exists
                    ...
                    # mark entry signal as acted (so it won't re-trigger)
...
async def super_signal_scanner_loop_5m():
    ...
    Detect when 2 or more of (1h,4h,1d) have last-candle flips within the same SUPER_WINDOW_SECONDS window,
    and emit a 'super' signal (priority). Avoid duplicates.
    """
    while True:
        try:
            symbols = await get_tradable_usdt_symbols()
            for symbol in symbols:
                if is_stablecoin_symbol(symbol) or (SKIP_DIGIT_PREFIX and _re_leading_digit.match(symbol)) or symbol_too_new(symbol):
                    continue
                flips = []
                # for each long TF check whether it flipped on its last closed candle and capture last_ts
                for tf in ["1h", "4h", "1d"]:
                    try:
                        flipped, last_ts = await detect_flip_on_last_candle(symbol, tf)
                        if flipped and last_ts:
                            flips.append((tf, last_ts))
                    except Exception:
                        continue
                if len(flips) >= 2:
                    # check whether the flips occurred within SUPER_WINDOW_SECONDS
...
async def log_current_root_signals():
    try:
        ...
        for key, sig in active_root_signals.items():
            ...
            log(f" {idx}. id={key} symbol={symbol} type={stype} root_tf={root_tf} status={status} priority={priority} components={comps} created_at={created}")
    except Exception as e:
...
async def debug_check_symbol(symbol: str = Query(..., min_length=1), tf: str = Query("1h"), _auth=Depends(require_admin_auth)):
    ...
    closes = candles_to_closes(dq)
    hist = macd_hist(closes)
    flip_on_last, last_ts = await detect_flip_on_last_candle(symbol, tf)
