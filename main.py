...
async def init_db():
    ...
    # Create tables (unchanged) then create uniqueness constraints / partial index
    ...
    # Prevent more than one open trade per symbol (partial index supported in SQLite)
    try:
        await db.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_open_trade_symbol ON trades(symbol) WHERE open = 1")
    except Exception:
        # Older SQLite builds may not support partial indexes; log and continue.
        log("Partial index for open trades not created (maybe older SQLite). Continuing without it.")
    await db.commit()
...
async def persist_root_signal(sig: Dict[str, Any]):
    ...
    If a row for (symbol, signal_type) exists, update its fields; otherwise insert.
    """
    ...
    try:
        ...
        # If this fails, log full repr to understand constraint errors
...
async def persist_trade(tr: Dict[str, Any]):
    try:
        ...
        log("persist_trade error:", repr(e))
        raise

async def try_create_trade_atomic(tr: Dict[str, Any]) -> bool:
    """
    Atomically insert new trade only if there is no open trade for the same symbol.
    Returns True if the trade was inserted, False if an open trade already existed.
    """
    try:
        sql = """
        INSERT INTO trades (id,symbol,side,qty,entry_price,sl_price,created_at,open,raw_response)
        SELECT ?,?,?,?,?,?,?,?,?
        WHERE NOT EXISTS (SELECT 1 FROM trades WHERE symbol = ? AND open = 1)
        """
        params = (
            tr["id"], tr["symbol"], tr["side"], tr["qty"], tr.get("entry_price"),
            tr.get("sl_price"), tr["created_at"], 1, json.dumps(tr.get("raw") or {}),
            tr["symbol"]
        )
        await db.execute(sql, params)
        await db.commit()
        # Check whether our trade exists now (insert succeeded)
        async with db.execute("SELECT COUNT(*) FROM trades WHERE id = ?", (tr["id"],)) as cur:
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    Add signal with duplication guard:
      ...
      - Re-check DB for existing signal of same (symbol,signal_type) inside lock to prevent race across instances
      - Persist to DB and register in-memory otherwise
    Uses per-symbol lock and DB unique index to avoid duplicates across concurrent instances.
    """
    sid = sig["id"]
    stype = sig.get("signal_type", "root")
    sym = sig["symbol"]
    ...
    async with lock:
        ...
        if signal_exists_for(sym, stype):
            ...
            return False
        # Check DB for existing signal (defensive, handles multi-instance race)
        try:
            async with db.execute("SELECT id FROM root_signals WHERE symbol = ? AND signal_type = ? LIMIT 1", (sym, stype)) as cur:
                row = await cur.fetchone()
            if row:
                log("Duplicate signal suppressed (db):", sym, stype, "existing id:", row[0])
                return False
        except Exception as e:
            log("DB check for existing signal failed:", repr(e))
            # fallthrough - try to add anyway
        ...
        try:
            ...
            log("persist_root_signal error after adding in-memory:", repr(e))
        log("Added signal:", sid, stype, sym)
        # Send telegram (dedupe inside send_telegram prevents immediate duplicates)
        try:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    # add_signal will perform duplication checks under lock and DB check
                        ...
                        if await has_open_trade(symbol):
                            ...
                            if TRADING_ENABLED:
                                try:
                                    ...
                                    if created:
                                        log("Placed real order and persisted", symbol, res)
                                        await send_telegram(f"Placed real order {symbol} qty={qty} side={side}")
                                    else:
                                        log("Real order placement persisted skipped because open trade exists for", symbol)
                                        # Optional: consider cancelling the order here if duplicates must be avoided at all costs
                                except Exception as e:
