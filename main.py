# main.py
# Bybit MACD multi-timeframe scanner — atomic DB inserts, startup dedupe/migrations,
# improved network resiliency, and notifications dedupe.
#
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    Add signal with duplication guard:
      - Attempt atomic DB-first insertion using try_create_signal_atomic()
      - Only register in-memory and notify if DB insert succeeded
    """
    sid = sig["id"]
    stype = sig.get("signal_type", "root")
    sym = sig["symbol"]
        ...
        try:
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    if added:
                        log("Root flip detected and added:", symbol, root_tf, last_start, "flip_kind:", flip_kind, "instance:", INSTANCE_ID)
                        if public_ws:
