...
# recent super keys to prevent duplicate SUPER signals
recent_super_keys: Dict[str, int] = {}  # key->created_at
...
# Small helper to run background tasks resiliently (auto-restart with backoff)
def start_resilient_task(coro_func, *args, name: str = None):
    """
    Start a resilient background task that will restart on exceptions.
    coro_func should be a coroutine function (callable).
    """
    async def _runner():
        backoff = 1
        while True:
            try:
                ...
                log(f"Resilient task {name or coro_func.__name__} cancelled")
                raise
            except Exception as e:
                log(f"Resilient task {name or coro_func.__name__} crashed:", e, "restarting in", backoff)
                await asyncio.sleep(backoff)
                backoff = min(60, backoff * 2)
            else:
                # If the task returns normally, restart after a short pause (shouldn't normally happen)
                await asyncio.sleep(1)
    try:
        return asyncio.create_task(_runner())
    except Exception as e:
        log("Failed to start resilient task:", e)
        return None
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    async with lock:
        ...
        # Immediately schedule an evaluation of this newly added signal so we don't wait for the periodic loop.
        try:
            # For root/super we prefetch required other TFs then evaluate so entry detection doesn't miss due to stale cache.
            if stype in ("root", "super"):
                asyncio.create_task(_prefetch_and_evaluate(sig))
            else:
                asyncio.create_task(evaluate_single_signal(sig))
        except Exception:
            pass

        return True

async def _prefetch_and_evaluate(sig: Dict[str, Any]):
    """
    Prefetch all required TF caches for this signal, then evaluate the signal for immediate MTF alignment.
    This avoids missing MTF alignment at the moment a root is added because other TFs weren't cached yet.
    """
    try:
        ...
        # Prefetch in parallel (bounded by PUBLIC_REQUEST_SEMAPHORE inside ensure_cached_candles)
        await asyncio.gather(*(ensure_cached_candles(symbol, tf, MIN_CANDLES_REQUIRED) for tf in required_tfs))
        # slight delay to allow caches to settle and any WS in-progress messages to be merged
        await asyncio.sleep(0.2)
        await evaluate_single_signal(sig)
    except Exception as e:
        log("_prefetch_and_evaluate error for", sig.get("id"), e)
...
async def process_inprogress_update(symbol: str, interval_token: str):
    ...
    If the kline token corresponds to a root timeframe (1h/4h) or participates in super detection (1h/4h/1d),
    attempt immediate detect_flip and create root/super signals if conditions met and flip stability + dedupe pass.

    This avoids missing mid-candle flips that occur after the script's periodic root scan.
    """
    try:
        ...
        if not tf:
            return
        # Only consider root-relevant frames for immediate root creation (1h,4h) and super detection (1h,4h,1d)
        ...
        # Require flip stability before creating root signal (configurable)
        ...
        # For 1d token we might contribute to super-signal creation: run centralized super detection to avoid duplication
        if tf == "1d":
            try:
                await try_create_super_for_symbol(symbol)
            except Exception as e:
                log("process_inprogress_update: try_create_super error", e)
            return
    except Exception as e:
        log("process_inprogress_update error for", symbol, interval_token, e)

# ---------- Super detection & dedupe helpers ----------
def make_super_key(symbol: str, flips: List[Tuple[str,int]]) -> str:
    """
    Create canonical super key from list of (tf, ts).
    Key uses min_ts and max_ts and sorted TF:ts components to prevent duplicates from different invocation orders.
    """
    if not flips:
        return f"SUPER-{symbol}-{int(time.time())}"
    min_ts = min(t for _, t in flips)
    max_ts = max(t for _, t in flips)
    comps = sorted([f"{tf}:{ts}" for tf, ts in flips])
    comp_s = "|".join(comps)
    return f"SUPER-{symbol}-{min_ts}-{max_ts}-{comp_s}"

def prune_recent_super_keys():
    """
    Remove old entries older than SUPER_WINDOW_SECONDS * 2 (buffer).
    """
    now_s = int(time.time())
    remove = []
    for k, t in recent_super_keys.items():
        if now_s - t > (SUPER_WINDOW_SECONDS * 2):
            remove.append(k)
    for k in remove:
        try:
            del recent_super_keys[k]
        except Exception:
            pass

async def try_create_super_for_symbol(symbol: str):
    """
    Centralized super creation logic:
      - detect open flips for 1h,4h,1d
      - if 2+ flips and within SUPER_WINDOW_SECONDS -> create super signal (deduped)
    """
    flips = []
    for tf in ("1h", "4h", "1d"):
        try:
            fk, ts = await detect_flip(symbol, tf)
            if fk == "open" and ts:
                flips.append((tf, ts))
        except Exception:
            continue
    ...
    if max(times) - min(times) > SUPER_WINDOW_SECONDS:
        return False
    # build canonical key
    key = make_super_key(symbol, flips)
    now_s = int(time.time())
    prune_recent_super_keys()
    if key in recent_super_keys:
        log("Super creation skipped (recent_super_keys dedupe):", key)
        return False
    # Additional dedupe across active signals
    if signal_exists_for(symbol, "super"):
        log("Super creation skipped (active super exists) for", symbol)
        # still record key to guard further repeats
        recent_super_keys[key] = now_s
        return False
    # Create super signal
    sig = {
        "id": key,
    ...
    if added:
        recent_super_keys[key] = now_s
        log("SUPER signal detected for", symbol, "components:", sig["components"], "key:", key)
        return True
    return False
    ...
    while True:
        try:
                ...
                async def _prewarm(sym):
                    async with semaphore:
                        try:
                            ...
                            pass
                await asyncio.gather(*[_prewarm(s) for s in symbols])
            except Exception as e:
                log("Prewarm error:", e)
                ...
                async with semaphore:
                    try:
                        ...
                        if added:
                            ...
                            # As soon as a root is added, also attempt to create super if it applies for this symbol
                            try:
                                await try_create_super_for_symbol(sym)
                            except Exception:
                                pass
                    except Exception as e:
            ...
            for symbol in symbols:
                # Note: global filters intentionally removed here to maximise discovery.
                tasks.append(asyncio.create_task(_detect_and_maybe_create(symbol)))

            if tasks:
                # run detection tasks but ensure they don't block forever
...
async def evaluate_single_signal(sig: Dict[str, Any]):
    """
    Evaluate a single signal (root or super) for entry readiness and attempt to place (or simulate) the trade.
    This isolates logic so new signals get evaluated immediately on add_signal.
    """
    try:
        stype = sig.get("signal_type", "root")
        symbol = sig.get("symbol")
        ...
        if is_stablecoin_symbol(symbol) or symbol_too_new(symbol):
            # if symbol not valid, remove the signal
            try:
                await remove_signal(sig.get("id"))
            except Exception:
                pass
            return
        ...
        tf_status = {}
        # For each timeframe determine: has/closed/positive/flip_last/last_ts
        ...
        if stype == "root":
            ...
            other_tfs = [tf for tf in required_tfs]
        ...
        if len(negatives) == 1:
            ...
            if not st.get("flip_last"):
                return
            # ensure all other tfs (excluding tf_to_flip) are positive
            for tf in other_tfs:
                if tf == tf_to_flip:
                    continue
        ...
        # Send explicit MTF alignment alert to Telegram immediately when entry added
        try:
            await send_telegram(f"MTF alignment confirmed: {symbol} root_tf={root_tf} -> entry created {entry_id}")
        except Exception:
            pass
        ...
        log("evaluate_single_signal error for", sig.get("id"), e)

async def evaluate_signals_loop():
    """
    Iterate active signals and evaluate if entry/trade conditions are met.
    This loop now delegates per-signal evaluation to evaluate_single_signal.
    """
    while True:
        try:
            ...
            for sid in ids:
                try:
                    ...
                    if not sig:
                        continue
                    # schedule per-signal evaluation but don't flood tasks: await each to keep order
                    await evaluate_single_signal(sig)
                except Exception as e:
...
async def super_signal_scanner_loop_5m():
    ...
    while True:
        try:
            ...
            for s in symbols:
                tasks.append(asyncio.create_task(_check_super(s)))
...
async def expire_signals_loop():
    ...
    Also prune recent_super_keys to avoid unbounded memory.
    """
    while True:
        try:
            now_s = int(time.time())
...
# ---------- Debug endpoints ----------
def _observed_registry_entries_for_symbol(symbol: str) -> List[Dict[str, Any]]:
    out = []
    try:
        for (sym, token, start), rec in observed_flip_registry.items():
            if sym == symbol:
                out.append({
                    "symbol": sym,
                    "token": token,
                    "tf": REVERSE_TF_MAP.get(token, token),
                    "start": start,
        ...
        pass
    return sorted(out, key=lambda x: (x["start"], x["token"]))

def _observed_registry_summary() -> Dict[str, Any]:
    """
    Return a summary of observed_flip_registry:
      - total_entries: int
      - per_symbol_counts: {symbol: count}
      - entries: list[...] (full list may be large)
    """
    summary = {"total_entries": 0, "per_symbol_counts": {}, "entries": []}
    try:
        for (sym, token, start), rec in observed_flip_registry.items():
            summary["total_entries"] += 1
            summary["per_symbol_counts"].setdefault(sym, 0)
            summary["per_symbol_counts"][sym] += 1
    ...
    summary["entries"] = sorted(summary["entries"], key=lambda x: (x["last_seen"] or 0), reverse=True)
    return summary
...
async def debug_current_roots(_auth=Depends(require_admin_auth)):
    """
    Extended debug view:
    - current in-memory persisted root/super/entry signals
    - observed_flip_registry summary (helps to see which in-progress flips were observed and their stability counters)
    """
    try:
        signals = list(active_root_signals.values())
        registry_summary = _observed_registry_summary()
        return {"count": len(signals), "signals": signals, "observed_flip_registry_summary": registry_summary}
    except Exception as e:
        log("debug_current_roots error:", e)
        return {"count": len(active_root_signals), "signals": list(active_root_signals.values()), "observed_flip_registry_summary": {}}
...
async def debug_check_symbol(symbol: str = Query(..., min_length=1), tf: str = Query("1h"), _auth=Depends(require_admin_auth)):
    ...
    dq = cache_get(symbol, token)
    if not dq:
        # still provide observed registry entries for this symbol even if no cache
        registry_entries = _observed_registry_entries_for_symbol(symbol)
        return {"symbol": symbol, "tf": tf, "klines_count": 0, "error": "no cached klines", "observed_flip_registry_entries": registry_entries}
    closes = candles_to_closes(dq)
    hist = macd_hist(closes)
    flip_kind, flip_ts = await detect_flip(symbol, tf)
    closed = last_candle_is_closed(dq, token)
    # provide observed flip registry info if available (all entries for the symbol)
    registry_info_all = _observed_registry_entries_for_symbol(symbol)
    # keep sample of last few klines
    sample_last = list(dq)[-5:]
    return {
        "symbol": symbol,
        "tf": tf,
        ...
        "flip_ts": flip_ts,
        "last_closed": closed,
        "sample_last_klines": sample_last,
        "observed_flip_registry_entries": registry_info_all,
    }
...
async def startup():
    ...
    # Use resilient runner so loops are restarted on unexpected exceptions
    start_resilient_task(root_scanner_loop, "1h", name="root_scanner_1h")
    start_resilient_task(root_scanner_loop, "4h", name="root_scanner_4h")
    start_resilient_task(super_signal_scanner_loop_5m, name="super_scanner_5m")
    start_resilient_task(evaluate_signals_loop, name="evaluate_signals")
    start_resilient_task(expire_signals_loop, name="expire_signals")
