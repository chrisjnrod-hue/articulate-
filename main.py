...
# ---------- Instruments fetch / symbol helpers ----------
async def get_tradable_usdt_symbols() -> List[str]:
    """
    Fetch instruments using several endpoints and return discovered tradable symbols.
    This function no longer applies any filtering — it collects symbols reported by the endpoints
    and caches symbol metadata for later use.
    """
    ...
    for ep in INSTRUMENTS_ENDPOINTS:
        ...
        for it in items:
            try:
                ...
                # Store whatever metadata we have for later; do not filter symbols here
...
# Provide minimal implementations if missing to avoid NameError.
# If you already have implementations elsewhere, these will be no-ops.
...
def last_candle_is_closed(dq: Deque[Dict[str, Any]], token: str, safety_seconds: int = 2) -> bool:
    """
    Best-effort check: if last candle has 'end' and end <= now - safety_seconds, consider closed.
    Some kline formats may not include 'end' -> treat as open.
    """
    try:
        last = list(dq)[-1]
        end = last.get("end")
        if not end:
            return False
        now_s = int(time.time())
        return (end <= now_s - safety_seconds)
    except Exception:
        return False
