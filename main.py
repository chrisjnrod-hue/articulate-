...
# Concurrency for fetching candles
CANDLE_FETCH_CONCURRENCY = int(os.getenv("CANDLE_FETCH_CONCURRENCY", "8"))
...
# Semaphore for controlling concurrent remote candle fetches (created on startup)
candle_fetch_semaphore: Optional[asyncio.Semaphore] = None
...
async def ensure_cached_candles(symbol: str, tf: str, required: int):
    """
    Ensure candles for symbol/tf are present in cache. This function now uses a semaphore
    to limit concurrent remote fetches so we don't overwhelm the public API.
    """
    token = TF_MAP[tf]
    dq = cache_get(symbol, token)
    if dq and len(dq) >= required and not cache_needs_refresh(symbol, token):
        return
    # Use shared semaphore if created; otherwise create a temporary local one.
    global candle_fetch_semaphore
    sem = candle_fetch_semaphore or asyncio.Semaphore(CANDLE_FETCH_CONCURRENCY)
    async with sem:
        # double-check after acquiring semaphore (another coroutine may have fetched)
        dq = cache_get(symbol, token)
        if dq and len(dq) >= required and not cache_needs_refresh(symbol, token):
            return
...
async def root_scanner_loop(root_tf: str):
    ...
    while True:
        try:
            ...
            # Prewarm only the root timeframe for all symbols first (parallel, concurrency-controlled)
            if symbols:
                try:
            ...
            for symbol in symbols:
                ...
                if info.get("launch_ts"):
                    ...
                    if age_days < (MIN_SYMBOL_AGE_MONTHS * 30):
                        continue
                # ensure cache for root tf (cheap now because prewarm ran)
                ...
                if added:
                    try:
                        if public_ws and getattr(public_ws, "detect_template", None):
                            # subscribe to small TFs so we get closed kline events for quick evaluation
...
async def evaluate_signal(sig_id: str):
    ...
    tf_status = {}
    # fetch all required tf candles in parallel (use semaphore inside ensure_cached_candles)
            ...
            try:
                ...
                try:
                    ...
                    for symbol, interval_token, candle, is_closed in entries:
                        # interval_token might be 'D' or numeric; keep existing mapping expectations
                        # convert known D to 'D' token in TF_MAP later usage; here we use token as-is
                        try:
                            # merge closed candles only when confirmed/closed
                            if is_closed:
                                # ensure token matches our cached keys: use interval_token as string
...
async def startup():
    global public_ws, candle_fetch_semaphore
    await init_db()
    # initialize semaphore for controlled concurrency
    candle_fetch_semaphore = asyncio.Semaphore(CANDLE_FETCH_CONCURRENCY)
