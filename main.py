...
#
# Notes:
# - Ensure .env contains TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, ADMIN_API_KEY, and optional BYBIT keys.
# - If you run multiple instances, add a distributed lock for trade placement to avoid cross-process duplicates.
...
# recent_root_signals: additional dedupe guard mapping symbol -> most recent root_flip_time we've created
recent_root_signals: Dict[str, int] = {}
# recent_super_signals: dedupe guard mapping symbol -> frozenset(component_flip_times)
recent_super_signals: Dict[str, Tuple[int, ...]] = {}
# recent_entry_signals: dedupe guard mapping symbol -> root_flip_time (or entry creation ts)
...
def merge_into_cache(symbol: str, interval_token: str, candles: List[Dict[str, Any]]):
    ...
    Candle dicts may include keys: start, end, open, close
    """
    if not candles:
        return
    ...
    for c in candles:
        if c["start"] not in existing_starts:
            # ensure we have at least start and close; open optional but should be present from updated fetch/ws
            dq.append(c)
...
async def fetch_klines(symbol: str, interval_token: str, limit: int = 200) -> List[Dict[str, Any]]:
    ...
    Returns list of candles as dicts with keys: start (seconds), end (seconds or None), open (float), close (float)
    ...
    for it in items:
        try:
            ...
            if isinstance(it, dict):
                ...
                # common klines arrays: [start, open, high, low, close, ...]
                if len(it) >= 5:
                    start = it[0]
                    open_val = it[1]
                    close_val = it[4]
            ...
            if open_val is not None:
                try:
                    ...
                    open_f = None
            # If open not available fall back to previous close in consumer logic if needed; prefer having open
...
async def detect_flip(symbol: str, tf: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    Detects flip and returns:
      - ("open", last_start) if the most recent candle is IN-PROGRESS (not closed) and
        it is the FIRST green (inprog_open < inprog_close) after the previous CLOSED red (prev_open > prev_close)
        AND MACD transitioned from prev_closed_hist < 0 to current_in_progress_hist > 0.

      - (None, None) otherwise.

    This enforces "open of first green candle after previous closed red" semantics.
    """
    token = TF_MAP[tf]
    ...
    if not dq or len(dq) < MIN_CANDLES_REQUIRED:
        return None, None
    # if last candle is closed, do not consider (we only react on open flips)
    if last_candle_is_closed(dq, token, safety_seconds=3):
        return None, None
    closes = candles_to_closes(dq)
    if len(closes) < MACD_SLOW + MACD_SIGNAL + 1:
        return None, None
    # Split: closed (confirmed) candles exclude in-progress last
    closed_closes = closes[:-1]
    inprog_close = closes[-1]
    # obtain opens for closed candles and in-progress (may be None if not provided)
    opens = candles_to_opens(dq)
    if len(opens) < 2:
        # we need at least previous closed open + inprog open (or at least previous open)
        prev_closed_open = None
        inprog_open = None
    else:
        prev_closed_open = opens[-2]
        inprog_open = opens[-1]
    # MACD hist arrays
    ...
    if prev is None or cur is None:
        return None, None
    # Strict MACD condition: previous closed hist must be < 0 and current in-progress hist > 0
    if not (prev < 0 and cur > 0):
        return None, None
    # Strict candle color checks:
    # previous closed candle must be red (prev_closed_close < prev_closed_open)
    # in-progress must be green (inprog_open < inprog_close)
    try:
        prev_closed_close = list(dq)[-2]["close"]
    except Exception:
        prev_closed_close = None
    # If we have both opens, use them strictly; if opens missing, fallback to comparing prev_closed_close < inprog_close (weaker)
    if prev_closed_open is not None and inprog_open is not None:
        ...
        # fallback: require inprog_close > prev_closed_close (weaker), but still only after MACD transition
        if prev_closed_close is None:
            return None, None
...
class PublicWebsocketManager:
    ...
    def _extract_kline_entries(self, obj: Dict[str, Any]):
        ...
        Candle dicts include start, end (if present), open (if present), close
        """
        out = []
        try:
            ...
            for entry in (data or []):
                # entry might be e.g. {"symbol":"BTCUSDT","interval":"5","start":"...","end":"...","open":"...","close":"...","confirm":True}
...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    Additional robust guards:
      ...
      - For super signals: dedupe by components tuple (flip times)
    """
    sid = sig["id"]
    ...
    async with lock:
        ...
        # Strong guard for super signals: dedupe by components' flip times
        if stype == "super":
            ...
            if comp_times and recent_super_signals.get(sym) == comp_times:
                ...
                return False
            # also check existing active super signals with identical component times
        ...
        # For super signals record recent components tuple to prevent immediate duplicates
        if stype == "super":
            comps = sig.get("components") or []
            try:
                comp_times = tuple(sorted(int(c.get("flip_time")) for c in comps if c.get("flip_time") is not None))
...
async def remove_signal(sig_id: str):
    ...
    if stype == "root":
        ...
        try:
            ...
            pass
    # If removing a super signal, clear recent_super_signals entry if it matches this components tuple.
    if stype == "super":
        comps = sig.get("components") or []
        try:
            comp_times = tuple(sorted(int(c.get("flip_time")) for c in comps if c.get("flip_time") is not None))
        ...
        try:
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    # add_signal will perform duplication checks under lock and validate it's current open root candle
...
async def evaluate_signals_loop():
    ...
    """
    # Note: macd_positive_from_closes might be defined elsewhere; if not, it's expected to be a simple check
    # We'll implement a small fallback here to avoid NameError.
        ...
        try:
            ...
            for sid in ids:
                try:
                    ...
                    # Root must be present and flipped
...
async def debug_check_symbol(symbol: str = Query(..., min_length=1), tf: str = Query("1h"), _auth=Depends(require_admin_auth)):
    ...
    return {
        ...
        "sample_opens": opens[-5:],
        "sample_closes": closes[-5:],
    }
...
async def admin_send_root_summary(_auth=Depends(require_admin_auth)):
    ...
    """
    # best-effort placeholder; implement send_root_signals_telegram if needed
    try:
        await send_telegram("Manual root summary requested")
    except Exception:
        pass
...
async def startup():
    ...
    # Super-signal scanner (driven by small-cadence loop)
