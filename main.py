...
# Websocket URLs (v5 per screenshot)
...
# Candidate REST endpoints (legacy fallbacks kept)
# We now prefer instruments-info endpoints for discovery (v5)
        ...
        if isinstance(maybe.get("list"), list):
            ...
            for v in maybe.values():
                if isinstance(v, list):
                    # further validate (list of dicts containing 'symbol')
                    if v and isinstance(v[0], dict) and ("symbol" in v[0] or "name" in v[0]):
                        result_list = v
                        break
