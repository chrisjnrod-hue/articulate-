...
async def add_signal(sig: Dict[str, Any]) -> bool:
    ...
    async with lock:
        ...
        try:
            # schedule without blocking; evaluation function will re-check dedupe and place entry if ready
...
def flip_is_stable_enough(symbol: str, tf: str, start: int) -> bool:
    ...
    if FLIP_STABILITY_SECONDS <= 0:
        return True
    token = TF_MAP.get(tf) if tf in TF_MAP else tf  # allow tf or token input
    # If token provided as token string (like '60'), map to tf first
    if token is None and tf in REVERSE_TF_MAP:
        token = tf
    if token is None:
        # fallback: attempt to find token from provided tf
