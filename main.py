...
async def detect_flip(symbol: str, tf: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    Strict logic: previous CLOSED histogram must be strictly negative (<0) and the in-progress histogram strictly positive (>0).
    This enforces "first green candle after previous closed red" semantics.
    ...
    if last_candle_is_closed(dq, token, safety_seconds=3):
        return None, None
    # closed_closes excludes in-progress last candle
    ...
    # Require valid computed histograms and non-None values for the relevant positions.
    if not hist_closed or not hist_with_inprog:
        return None, None
    prev = hist_closed[-1]
    cur = hist_with_inprog[-1]
    # Enforce strict negative -> positive transition and require both to be not None
