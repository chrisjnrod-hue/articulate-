# main.py
# Bybit MACD multi-timeframe scanner — updated with robust endpoint probing, v5 websocket URLs,
# strict USDT perpetual symbol filtering, protected debug checks, and safer private-feature guards.
...
# Private WS URL chosen only when private features are enabled
def private_features_enabled() -> bool:
    # private features require TRADING_ENABLED and API keys to be present
    return TRADING_ENABLED and bool(BYBIT_API_KEY) and bool(BYBIT_API_SECRET)
...
# Public endpoints to probe (expanded to include common v5 names)
PUBLIC_ENDPOINT_CANDIDATES = [
    "/v5/market/symbols",
    "/v5/market/instruments-info",  # sometimes used to list instruments
    "/v5/market/tickers",
    "/v2/public/symbols",
    "/v2/public/tickers",
]
...
# ---------- Resilient public GET (improved with body logging) ----------
async def resilient_public_get(endpoints: List[str], params: Dict[str, Any] = None, timeout: int = 12) -> Optional[Dict[str, Any]]:
    """
    Try endpoints against API_HOSTS. Log status and truncated response body for non-200 to aid diagnosis.
    Returns first successful JSON-decoded result or None.
    """
    params = params or {}
    for host in API_HOSTS:
        ...
        for ep in endpoints:
            ...
            url = host + path
            try:
                r = await httpx_client.get(url, params=params, timeout=timeout)
            except Exception as e:
            ...
            if r.status_code == 200:
                try:
                    ...
                    try:
                        txt = (await r.text())[:1000]
                        log("Public GET returned non-json body (truncated):", txt)
                    except Exception:
                        log("Public GET returned non-json and could not read text")
                    continue
            else:
                try:
                    txt = (await r.text())[:1000]
                    log("Public GET", url, "status", r.status_code, "body (truncated):", txt)
                except Exception as e:
                    log("Public GET", url, "status", r.status_code, "and failed reading body:", e)
                # try next endpoint/host
    return None

# ---------- Signed request (guard when keys are missing) ----------
...
async def bybit_signed_request(method: str, endpoint: str, payload: Dict[str, Any] = None):
    if not BYBIT_API_KEY or not BYBIT_API_SECRET:
        log("Attempted signed request without API keys; aborting:", endpoint)
        return {"retCode": -1, "retMsg": "API keys not configured"}
    ts = str(int(time.time() * 1000))
    body = json.dumps(payload) if payload else ""
    signature = bybit_sign_v5(BYBIT_API_SECRET, ts, method.upper(), endpoint, body)
    headers = {
...
async def get_tradable_usdt_symbols() -> List[str]:
    """
    Discover tradable USDT perpetual (linear) contract symbols from Bybit public endpoints.

    Heuristics:
    - Must be USDT quoted (symbol endswith USDT or explicit quoteCoin == USDT).
    - Must appear to be perpetual/linear/swap via common fields.
    - Must not have a stablecoin as the base (skip weird pairs).
    """
    ...
    # Normalize possible shapes: v5 often returns {"result": {"list": [...]}} or {"result": [...]}
    result_list = []
    maybe = res if not isinstance(res, dict) else (res.get("result") or res.get("data") or res.get("symbols") or res.get("list") or res)
    ...
    for item in result_list:
        try:
            ...
            if isinstance(item, dict):
                name = item.get("symbol") or item.get("name") or item.get("s") or item.get("symbolName") or item.get("symbol_name")
            else:
                name = str(item)
            if not name:
                continue
            ...
            quote_coin = None
            if isinstance(item, dict):
                quote_coin = item.get("quoteCoin") or item.get("quote_coin") or item.get("quote") or item.get("quoteCurrency")
            is_usdt_pair = False
            if name.upper().endswith("USDT"):
                is_usdt_pair = True
            if isinstance(quote_coin, str) and quote_coin.upper() == "USDT":
                is_usdt_pair = True
            if not is_usdt_pair:
                continue

            # Heuristics to detect perpetual/linear contracts
            is_perpetual = False
            if isinstance(item, dict):
                cat = str(item.get("category") or item.get("contract_type") or item.get("contractType") or item.get("type") or item.get("symbolType") or "").lower()
                if any(k in cat for k in ("perpetual", "linear", "swap")):
                    is_perpetual = True
                ctype = str(item.get("contractType") or item.get("contract_type") or "").lower()
                if any(k in ctype for k in ("perpetual", "swap")):
                    is_perpetual = True
                st = str(item.get("symbolType") or "").lower()
                if "perpetual" in st:
                    is_perpetual = True
                if "isInverse" in item and isinstance(item.get("isInverse"), bool):
                    if item.get("isInverse") is False:
                        is_perpetual = True
                # textual search fallback
                for v in item.values():
                    if isinstance(v, str) and any(k in v.lower() for k in ("perpetual", "swap", "linear")):
                        is_perpetual = True
                        break
            else:
                is_perpetual = True
            ...
            status = None
            if isinstance(item, dict):
                status = (item.get("status") or item.get("state") or item.get("trade_status") or item.get("symbolStatus") or "").lower()
            if status and status not in ("trading", "normal", "enabled", "ok", "active"):
                continue

            # skip stablecoin bases
            base = name[:-4] if name.upper().endswith("USDT") else None
            if base and base.upper() in STABLECOINS:
                continue
    ...
    log("Discovered", len(symbols), "USDT perpetual symbols")
...
@app.get("/debug/check_endpoints")
async def debug_check_endpoints(_auth=Depends(require_admin_auth)):
    """
    Probes PRIMARY_API_HOST for each PUBLIC_ENDPOINT_CANDIDATES and returns status + sample body.
    Protected by ADMIN_API_KEY.
    """
    results = {}
    for ep in PUBLIC_ENDPOINT_CANDIDATES:
        host = PRIMARY_API_HOST.rstrip("/")
        path = ep if ep.startswith("/") else f"/{ep}"
        url = host + path
        try:
            r = await httpx_client.get(url, timeout=8)
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                body = "<unreadable>"
        results[url] = {"status": r.status_code, "body_sample": body}
    return {"primary_host": PRIMARY_API_HOST, "results": results}

@app.post("/debug/clear_symbols_cache")
async def debug_clear_symbols(_auth=Depends(require_admin_auth)):
    global symbols_cache
    symbols_cache = None
    return {"cleared": True}
...
# small root endpoint to avoid HEAD/GET 404 at /
@app.get("/")
async def root():
...
@app.head("/")
async def root_head():
    # simple HEAD support for platform probes
    return {}
...
async def toggle(req: ToggleRequest, _auth=Depends(require_admin_auth)):
    ...
    if req.trading_enabled is not None:
        ...
        if TRADING_ENABLED and BYBIT_API_KEY and BYBIT_API_SECRET:
...
async def startup():
    global public_ws, private_ws, PRIVATE_WS_URL
    await init_db()
    ...
    log("Startup config:",
        ...
        "TRADING_ENABLED=", TRADING_ENABLED,
        "PRIVATE_FEATURES_ENABLED=", private_features_enabled(),
        "API_HOSTS=", API_HOSTS,
    ...
    # Only create and start private websocket manager when trading is enabled and API keys are present
    if private_features_enabled():
        PRIVATE_WS_URL = "wss://stream.bybit.com/v5/private" if BYBIT_USE_MAINNET else "wss://stream-testnet.bybit.com/v5/private"
        private_ws = PrivateWebsocketManager(PRIVATE_WS_URL, BYBIT_API_KEY, BYBIT_API_SECRET)
        asyncio.create_task(private_ws.connect_and_auth())
    else:
        # ensure private_ws remains None if not enabled
        private_ws = None
