...
# Added: periodic / on-change logging of current root signals and DB load on startup.
...
# how often to log active root signals (secs)
...
symbols_cache: Optional[List[str]] = None
symbols_cache_ts: int = 0
symbols_cache_ttl = 60  # seconds cache for instrument list
...
# log current root signals (safe, readable)
async def log_current_root_signals():
    try:
        ...
        if count == 0:
            ...
            return
        log(f"Current root signals: {count} (showing up to 20)")
        i = 0
        for key, sig in list(active_root_signals.items()):
            if i >= 20:
                break
            # try to log useful fields safely
            symbol = sig.get("symbol")
            root_tf = sig.get("root_tf")
            status = sig.get("status")
            flip_time = sig.get("root_flip_time") or sig.get("root_flip_candle_end") or sig.get("root_flip_ts")
            flip_price = sig.get("root_flip_price")
            created = sig.get("created_at")
            log(f"  {i+1}. id={key} symbol={symbol} root_tf={root_tf} status={status} flip_time={flip_time} price={flip_price} created_at={created}")
            i += 1
        if count > 20:
            log(f"  ... and {count-20} more")
    except Exception as e:
        log("log_current_root_signals error:", e)

async def root_signals_logger_loop():
    while True:
        try:
            await log_current_root_signals()
        except Exception as e:
            log("root_signals_logger_loop error:", e)
        await asyncio.sleep(max(10, ROOT_SIGNALS_LOG_INTERVAL))
...
async def resilient_public_get(endpoints: List[str], params: Dict[str, Any] = None, timeout: int = 12) -> Optional[Dict[str, Any]]:
    """
    Try endpoints across hosts. Returns first successful JSON body.
    """
...
async def load_persisted_root_signals():
    """
    Load persisted root signals from DB into active_root_signals on startup.
    """
    try:
        async with db.execute("SELECT id,symbol,root_tf,flip_time,flip_price,status,created_at FROM root_signals") as cur:
...
async def get_tradable_usdt_symbols() -> List[str]:
    ...
    Robust to several response shapes. Uses a small TTL cache to avoid hammering API.
    """
    global symbols_cache, symbols_cache_ts
    now_ts = int(time.time())
    if symbols_cache and (now_ts - symbols_cache_ts) < symbols_cache_ttl:
    ...
    for ep in INSTRUMENTS_ENDPOINTS:
        ...
        # locate list of items inside many possible keys
        items = []
        ...
        if not items:
            if isinstance(resp, dict) and "ret_msg" in resp and "result" in resp:
                items = resp.get("result") or []
        if not items:
            last_resp_excerpt = body_excerpt
            log("Instruments response from", url, "did not contain expected list keys; excerpt:", body_excerpt[:400])
            continue

        for it in items:
            try:
                symbol = (it.get("symbol") or it.get("name") or it.get("code") or it.get("symbolName") or "").strip() if isinstance(it, dict) else ""
                quote = (it.get("quoteCoin") or it.get("quoteAsset") or it.get("quote_currency") or it.get("quote") or it.get("quoteCurrency") or "").upper() if isinstance(it, dict) else ""
                status = (it.get("status") or it.get("state") or it.get("is_trading") or it.get("active") or "").lower() if isinstance(it, dict) else ""
                inst_type = (it.get("instType") or it.get("type") or it.get("contractType") or "").upper() if isinstance(it, dict) else ""
                if not symbol:
                    # fallback for list-shaped items (rare in instruments responses)
                    continue
                if symbol.upper().endswith("USDT") or quote == "USDT":
    ...
    symbols_cache = uniq
    symbols_cache_ts = int(time.time())
    if not uniq:
        log("get_tradable_usdt_symbols: FOUND 0 symbols. Last instruments response excerpt:", (last_resp_excerpt[:400] + "...") if last_resp_excerpt else "<empty>")
    else:
        log("get_tradable_usdt_symbols: Found", len(uniq), "symbols (showing up to 10):", uniq[:10])
    return uniq

async def fetch_klines(symbol: str, interval_token: str, limit: int = 50) -> List[Dict[str, Any]]:
    """
    Robust kline fetch: supports v5 dict-shaped candles and older list-shaped candles.
    - interval_token should be TF_MAP value ('5','15','60', ...)
    - Returns list of dicts with keys: start (seconds), end (seconds or None), close (float)
    """
...
# ---------- WebSocket managers (Public and Private) ----------
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                ...
                if not klines:
                    continue
                last_candle_end = klines[-1].get("end") or klines[-1].get("start")
                ...
                if flip_idx is not None:
                    ...
                    sig = {"id": key, "symbol": symbol, "root_tf": root_tf, "root_flip_time": flip_candle["start"], "root_flip_candle_end": flip_candle.get("end"), "root_flip_price": flip_candle["close"], "created_at": now_ts_ms(), "status": "watching"}
...
async def startup():
    ...
    await init_db()
    # load persisted root signals into memory
    ...
    # instantiate public WS and ensure detection / template discovery before scanners start
    ...
    # start periodic logger for root signals
    asyncio.create_task(root_signals_logger_loop())
    ...
    # log current persisted signals right away
    await log_current_root_signals()
