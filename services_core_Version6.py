...
# The missing shared constants that scanners.py expects:
...
# Wider set of instruments endpoints (tolerant fallbacks)
...
# ---------- Startup instrumentation state ----------
STARTUP_STATE: Dict[str, Any] = {
    "db_initialized": False,
    "mtf_module_imported": False,
    "mtf_callback_registered": False,
    "mtf_task_scheduled": False,
}
LAST_STARTUP_EXCEPTION: Optional[str] = None
...
async def init_db():
    ...
    global db, STARTUP_STATE, LAST_STARTUP_EXCEPTION
    try:
        ...
        STARTUP_STATE["db_initialized"] = True
        LAST_STARTUP_EXCEPTION = None
        log("DB initialized at", DB_PATH)
    except Exception as e:
        LAST_STARTUP_EXCEPTION = str(e)
        STARTUP_STATE["db_initialized"] = False
        log("init_db error:", e)
...
def register_mtf_notify_callback(cb: Callable[[Dict[str, Any]], Any]):
    ...
    global _mtf_notify_cb, STARTUP_STATE
    _mtf_notify_cb = cb
    STARTUP_STATE["mtf_callback_registered"] = True
...
# ---------- Robust tradable symbols implementation ----------
async def get_tradable_usdt_symbols() -> List[str]:
    """
    Return list of symbols that are linear PERPETUAL and quote USDT.
    Tolerant parser that handles multiple API response shapes and field names.
    """
    global symbols_info_cache
    found: List[str] = []
    last_resp_excerpt = None

    # Query the instruments endpoints using resilient_public_get (handles retries)
    resp = await resilient_public_get(INSTRUMENTS_ENDPOINTS, timeout=12)
    if not resp:
        log("get_tradable_usdt_symbols: no response from instruments endpoints")
        return []
    ...
    try:
        ...
        try:
            last_resp_excerpt = json.dumps(resp)[:800]
        except Exception:
            try:
                last_resp_excerpt = str(resp)[:800]
            except Exception:
                last_resp_excerpt = "<unprintable>"
        ...
        for it in items:
            try:
                ...
                # Robust symbol extraction - accept several possible keys used across APIs
                symbol = (it.get("symbol") or it.get("name") or it.get("code") or it.get("symbolName") or it.get("instId") or "").strip()
                ...
                # Accept only USDT quotation (either symbol name endswith USDT or quote field says USDT)
                if not (symbol.upper().endswith("USDT") or quote == "USDT"):
                    continue

                base = symbol[:-4] if symbol.upper().endswith("USDT") else symbol
                # skip stablecoins themselves
                if base.upper() in STABLECOINS:
                    continue

                # Accept a range of status values; if status provided and looks like non-trading skip
                if status and isinstance(status, str):
                    if status not in ("trading", "listed", "normal", "active", "open"):
                        # not a trading/listed instrument
                        continue

                # Save metadata for later filters
                symbols_info_cache[symbol] = {"symbol": symbol, "quote": quote, "inst_type": inst_type, "status": status}
                found.append(symbol)
            except Exception:
                # tolerate malformed item
                continue

    except Exception as e:
        log("get_tradable_usdt_symbols parse error:", e, "excerpt:", last_resp_excerpt)
        return []
...
def init_app(fastapi_app):
    ...
    global app, _TELEGRAM_WORKER_TASK, _MTF_MONITORING_TASK, STARTUP_STATE, LAST_STARTUP_EXCEPTION
    app = fastapi_app
    ...
    @app.get("/debug/startup_status")
    async def debug_startup_status(_auth=Depends(require_admin_auth)):
        """Return startup state / diagnostics useful to debug MTF and DB init."""
        mtf_task_running = False
        try:
            mtf_task_running = _MTF_MONITORING_TASK is not None and not _MTF_MONITORING_TASK.done()
        except Exception:
            mtf_task_running = False
        return {
            "db_initialized": STARTUP_STATE.get("db_initialized", False),
            "mtf_module_imported": STARTUP_STATE.get("mtf_module_imported", False),
            "mtf_callback_registered": STARTUP_STATE.get("mtf_callback_registered", False),
            "mtf_task_scheduled": STARTUP_STATE.get("mtf_task_scheduled", False),
            "mtf_task_running": mtf_task_running,
            "last_startup_exception": LAST_STARTUP_EXCEPTION,
        }

    @app.get("/debug/mtf_status")
    async def debug_mtf_status(_auth=Depends(require_admin_auth)):
        """
        Return runtime status about the MTF module and monitoring task.
        Shows:
         - whether services_mtf import succeeded,
         - whether the MTF notify callback was registered,
         - whether the MTF task was scheduled and if it is running,
         - last startup exception (if any),
         - number of active root signals.
        """
        try:
            # Try to import services_mtf to detect import-time issues
            try:
                import services_mtf as _mtf  # noqa: F401
                mtf_imported = True
                mtf_import_error = None
            except Exception as e:
                mtf_imported = False
                mtf_import_error = str(e)

            task_repr = None
            task_running = False
            try:
                task = _MTF_MONITORING_TASK
                task_repr = repr(task)
            ...
            return {
                "mtf_module_imported": mtf_imported,
                "mtf_import_error": mtf_import_error,
                "mtf_callback_registered": STARTUP_STATE.get("mtf_callback_registered", False),
                "mtf_task_scheduled": STARTUP_STATE.get("mtf_task_scheduled", False),
                "mtf_task_repr": task_repr,
                "mtf_task_running": task_running,
                "last_startup_exception": LAST_STARTUP_EXCEPTION,
                "active_root_signals_count": len(active_root_signals),
            }
        except Exception as e:
            return {"error": "debug_mtf_status exception", "detail": str(e), "trace": traceback.format_exc()}

    @app.post("/debug/force_mtf")
    async def debug_force_mtf(_auth=Depends(require_admin_auth)):
        """
        Attempt to import the services_mtf module (if not present) and start the monitoring loop.
        Use this to force-start the simplified MTF monitoring loop in the running process.
        """
        global _MTF_MONITORING_TASK, STARTUP_STATE, LAST_STARTUP_EXCEPTION
        try:
            try:
                import services_mtf as _mtf
            except Exception as e:
                LAST_STARTUP_EXCEPTION = str(e)
                STARTUP_STATE["mtf_module_imported"] = False
                return {"ok": False, "error": "import services_mtf failed", "exception": str(e), "trace": traceback.format_exc()}

            STARTUP_STATE["mtf_module_imported"] = True

            # If a task already exists and is running, return that status
            try:
                if _MTF_MONITORING_TASK is not None and not _MTF_MONITORING_TASK.done():
                    return {"ok": True, "message": "MTF monitoring loop already running", "task": repr(_MTF_MONITORING_TASK)}
            except Exception:
                pass
            ...
            try:
                ...
                STARTUP_STATE["mtf_task_scheduled"] = True
                STARTUP_STATE["mtf_callback_registered"] = STARTUP_STATE.get("mtf_callback_registered", False) or True
                return {"ok": True, "message": "MTF monitoring loop started", "task": repr(_MTF_MONITORING_TASK)}
            except Exception as e:
                LAST_STARTUP_EXCEPTION = str(e)
                STARTUP_STATE["mtf_task_scheduled"] = False
                return {"ok": False, "error": "failed to start monitoring loop", "exception": str(e), "trace": traceback.format_exc()}

        except Exception as e:
            LAST_STARTUP_EXCEPTION = str(e)
            return {"ok": False, "error": "debug_force_mtf exception", "detail": str(e), "trace": traceback.format_exc()}
    ...
    async def startup_event():
        global public_ws, _TELEGRAM_WORKER_TASK, _MTF_MONITORING_TASK, STARTUP_STATE, LAST_STARTUP_EXCEPTION
        try:
            await init_db()
            await load_persisted_root_signals()
            STARTUP_STATE["db_initialized"] = STARTUP_STATE.get("db_initialized", False)
            log("Startup config:", "BYBIT_USE_MAINNET=", BYBIT_USE_MAINNET, "TRADING_ENABLED=", TRADING_ENABLED)
        except Exception as e:
            LAST_STARTUP_EXCEPTION = str(e)
            log("startup_event init/load error:", e)
