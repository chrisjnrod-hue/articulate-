.
# scanners.py - FIXED TIMING
# Complete implementation with proper interval execution
...
# ✅ FIX:  Continuous subscription maintenance with proper interval
async def _continuous_root_signal_subscriptions():
    ...
    while True:
        ...
        try:
            ...
            # Also maintain root TF subscriptions for discovery
            try:
        ...
        # ✅ FIX: Sleep remainder of 30s interval (NOT FIXED 30s)
        elapsed = time.time() - iteration_start
        sleep_for = max(1, 30 - elapsed)
        await asyncio.sleep(sleep_for)


# ✅ FIX: Continuous per-scan-interval kline fetching with PROPER timing
async def _continuous_kline_fetcher(root_tf: str):
    ...
    while True:
        ...
        # ✅ FIX: Sleep remainder of SCAN_INTERVAL_SECONDS
        elapsed = time.time() - iteration_start
...
# ✅ FIX:  Root scanner with PROPER timing
async def root_scanner_loop(root_tf: str):
    """Main root flip detection loop with guaranteed scan intervals."""
    import services as s
    s.log(f"Root scanner started for {root_tf}, interval={s. SCAN_INTERVAL_SECONDS}s, DISCOVERY_CONCURRENCY={s. DISCOVERY_CONCURRENCY}")
    ...
    while True:
        ...
        try:
            ...
            if not symbols:
                ...
                # Detect and create signals in parallel
                tasks = []
                ...
                elapsed_work = time.time() - iteration_start
                s.log(f"Root scanner {root_tf} iteration completed in {elapsed_work:.2f}s with {len(symbols)} symbols")

        except Exception as e:
            s.log(f"root_scanner_loop {root_tf} error: {e}")

        # ✅ FIX: Sleep remainder of SCAN_INTERVAL_SECONDS (not fixed sleep)
        elapsed = time.time() - iteration_start
        sleep_for = max(1, s.SCAN_INTERVAL_SECONDS - elapsed)
        await asyncio.sleep(sleep_for)


# ✅ FIX:  Evaluate signals with PROPER timing
async def evaluate_signals_loop():
    """Evaluate root signals for entry conditions with proper timing."""
    import services as s
    s.log("evaluate_signals_loop started, interval=half SCAN_INTERVAL_SECONDS")
    ...
    while True:
        ...
        try:  
            ...
            for sid in ids:
                try:
                    ...
                    try:
                        await s.send_telegram(f"✅ ENTRY READY: {symbol} root={root_tf} conditions aligned!")
                    except Exception:  
                        pass
                    ...
                    async with lock:
                        if await s.has_open_trade(symbol):
                            ...
                            if s. TRADING_ENABLED:
                                ...
                                await s.send_telegram(f"📊 SIMULATED TRADE:  {symbol} qty={qty} side={side}")
        ...
        # ✅ FIX: Sleep remainder of half scan interval
        elapsed = time.time() - iteration_start
        sleep_for = max(1, (s.SCAN_INTERVAL_SECONDS // 2) - elapsed)
        await asyncio.sleep(sleep_for)


# ✅ FIX: Expire signals with PROPER timing
