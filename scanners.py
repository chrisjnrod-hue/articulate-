# scanners.py - COMPLETE PRODUCTION VERSION
# MTF root signal detection with aggressive cache retry logic
        ...
        for sig in list(s.active_root_signals.values()):
            if sig.get("symbol") == symbol and sig.get("signal_type") == "root":
                try:
                    ...
                    s.log(f"_check_root_alignment error for {symbol}: {e}")
    except Exception: 
        pass
...
async def _evaluate_root_signal_for_entry(sig: Dict[str, Any]):
    """Evaluate a single root signal for MTF alignment with aggressive cache retry."""
    import services as s
    try:
        ...
        if stype != "root" or not symbol or not root_tf:
            s.log(f"❌ MTF eval SKIP {symbol}:  invalid stype/symbol/root_tf")
            return
        ...
        s.log(f"📥 FETCHING all TFs for {symbol}...")
        for tf in required_tfs:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if dq and len(dq) >= s.MIN_CANDLES_REQUIRED:
                        s. log(f"  ✓ Fetched {symbol} {tf} (attempt {attempt+1}) - {len(dq)} candles")
                        break
                    else:
                        s.log(f"  ⚠ {symbol} {tf} (attempt {attempt+1}) - only {len(dq) if dq else 0} candles, retrying...")
                        await asyncio.sleep(0.5)
                except Exception as e: 
                    s.log(f"  ✗ Fetch error {symbol} {tf} (attempt {attempt+1}): {e}")
                    if attempt < max_retries - 1:
                        await asyncio. sleep(0.5)

        # ✅ Now gather and log ALL TF status
        tf_status = {}
        for tf in required_tfs:
            token = s.TF_MAP[tf]
            dq = s.cache_get(symbol, token)

            s.log(f"  Checking {symbol} {tf}:  token={token}, dq_exists={bool(dq)}, candles={len(dq) if dq else 0}")
            ...
            if not dq or len(dq) < s.MIN_CANDLES_REQUIRED:
                tf_status[tf] = {
                    ...
                    "required": s.MIN_CANDLES_REQUIRED
                }
                s.log(f"    ❌ FAILED:  {len(dq) if dq else 0}/{s.MIN_CANDLES_REQUIRED} candles")
                continue
            ...
            try:
                ...
                flip_kind, flip_ts = await s.detect_flip(symbol, tf)
                flip_last = (flip_kind == "open")
                hist = s.macd_hist(closes)
                ...
                s.log(f"    ✓ OK: positive={positive}, flip={flip_kind}, macd_last={macd_last:. 6f}, candles={len(dq)}")
            except Exception as e: 
                s.log(f"    ✗ ANALYSIS ERROR: {e}")
                tf_status[tf] = {"has": False, "reason": f"analysis_error:{e}"}
                continue

        # ✅ Log full status
        s.log(f"  📊 Full TF Status: {tf_status}")

        # ✅ Check alignment conditions
        root_status = tf_status.get(root_tf, {})
        s.log(f"  Root TF {root_tf} status: has={root_status.get('has')}, flip_last={root_status.get('flip_last')}, positive={root_status.get('positive')}")

        if not root_status.get("has"):
            s.log(f"❌ MTF eval FAIL {symbol}:  root TF {root_tf} missing or insufficient cache ({root_status.get('candles', 0)}/{s.MIN_CANDLES_REQUIRED})")
            return
        ...
        if len(negatives) == 0:
            ...
            s.log(f"  ✅ Alignment:  All TFs positive")
        elif len(negatives) == 1:
            tf_to_flip = negatives[0]
            st = tf_status.get(tf_to_flip, {})
            s.log(f"  One negative TF {tf_to_flip}:  has={st.get('has')}, flip_last={st.get('flip_last')}")
            ...
            if st.get("has") and st.get("flip_last"):
                ...
                if remaining_positive: 
                    ...
                    s.log(f"  ✅ Alignment: {tf_to_flip} flipping, others positive")
                else:
                    alignment_reason = f"ONE_FLIPPING_BUT_OTHERS_NEGATIVE"
                    s.log(f"  ❌ Alignment: {tf_to_flip} flipping but others negative")
            else:
                alignment_reason = f"ONE_NEGATIVE_NO_FLIP({tf_to_flip})"
                s.log(f"  ❌ Alignment: {tf_to_flip} negative and no flip")
        else:
            alignment_reason = f"TOO_MANY_NEGATIVE({len(negatives)})"
            s.log(f"  ❌ Alignment: Too many negative TFs:  {negatives}")

        if not alignment_ok:
            s. log(f"❌ MTF eval FAIL {symbol}: reason={alignment_reason}")
            return

        s.log(f"✅ MTF ALIGNMENT SUCCESS {symbol} {root_tf}:  {alignment_reason}")
        ...
        s.log(f"❌ _evaluate_root_signal_for_entry EXCEPTION: {e}")
        import traceback
...
async def _continuous_kline_fetcher(root_tf: str):
    ...
    while True:
        ...
        try:
            symbols = await s.get_tradable_usdt_symbols()
            if not symbols:
                s.log(f"Kline fetcher {root_tf}:  no symbols")
            else:
                ...
                s.log(f"Kline refresh {root_tf}: {len(symbols)} symbols in {elapsed_work:.2f}s")

        except Exception as e:
            s.log(f"Kline fetcher error {root_tf}: {e}")
...
async def _ensure_root_signal_subscriptions(signal_dict: Dict[str, Any]):
    ...
    try:
        ...
        if not s.public_ws or not getattr(s.public_ws, "conn", None):
            s.log(f"Public WS not ready for {symbol}")
            return
...
async def root_scanner_loop(root_tf: str):
    ...
    while True:
        ...
        try:
            ...
            if not symbols:
                s.log(f"Root scanner {root_tf}: no symbols")
            else:
                try:
