...
class PublicWebsocketManager:
    ...
    async def subscribe_kline(self, symbol: str, interval_token: str) -> bool:
        """
        Subscribe to a kline topic; idempotent. Returns True on (attempted) success, False on failure.
        """
    ...
    async def unsubscribe_kline(self, symbol: str, interval_token: str) -> bool:
        """
        Unsubscribe from a kline topic and remove it from subscribed_topics.
        Returns True if unsubscribed or topic not present, False on failure.
        """
        topic = f"klineV2.{interval_token}.{symbol}"
        if topic not in self.subscribed_topics:
            return True
        if not self.conn:
            # nothing to do if not connected; remove locally
            try:
