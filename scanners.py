...
class PublicWebsocketManager:
    ...
    async def unsubscribe_kline(self, symbol: str, interval_token: str):
        """
        Unsubscribe a previously subscribed kline topic. Safe/idempotent.
        """
        topic = f"klineV2.{interval_token}.{symbol}"
        ...
        if not self.conn:
            ...
            try:
                import services as s
                s.log("WS unsubscribe skipped (no conn) for", topic)
            except Exception:
                pass
            # still remove from local set to avoid reattempting
            try:
