...
class PublicWebsocketManager:
    ...
    async def unsubscribe_kline(self, symbol: str, interval_token: str):
        ...
        if topic not in self.subscribed_topics:
            # still keep local state consistent
    ...
    async def _recv_loop(self):
        ...
        # import only services names that actually live in services
...
async def evaluate_signals_loop():
    """Evaluate root signals for MTF entry conditions - MORE FREQUENT CHECKS"""
    import services as s
        ...
        try:
            ...
            for sid in ids:
                try:
                    ...
                    # ✅ Check alignment for ANY status that isn't 'entry'
                    if status in ("watching", "monitoring", "pending"):
                        try:
                            await s.notify_alignment_if_ready(sig)
                        except Exception as e:
            ...
            # ✅ Check every 2-3 seconds (faster re-evaluation)
            await asyncio.sleep(max(2, s.SCAN_INTERVAL_SECONDS // 30))
