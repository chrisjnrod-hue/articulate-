# scanners.py
# Scanner loops + WebSocket manager + in-progress processing and super logic.
...
async def process_inprogress_update(symbol: str, interval_token: str):
    ...
    try:
        ...
        # tf == "1d" -> consider super detection based on flips in 1h/4h/1d
        if tf == "1d":
            flips = []
            for check_tf in ("1h", "4h", "1d"):
                try:
                    fk, ts = await s.detect_flip(symbol, check_tf)
                    if fk == "open" and ts:
            ...
            if len(flips) >= 2:
                ...
                max_t = max(times)
                min_t = min(times)
                if (max_t - min_t) <= s.SUPER_WINDOW_SECONDS:
                    # dedupe per-symbol recent_super_signals
                    last_super = s.recent_super_signals.get(symbol)
                    if last_super and (max_t - last_super) <= s.SUPER_WINDOW_SECONDS:
                        s.log("WS mid-candle: skipped SUPER (recent) for", symbol)
                        return
                    if s.signal_exists_for(symbol, "super"):
                        s.log("WS mid-candle: skipped super creation (already active) for", symbol)
                        return
                    key = f"SUPER-{symbol}-{max_t}"
                    sig = {
                        "id": key,
                        "symbol": symbol,
                        "root_tf": "multi",
                        "root_flip_time": max_t,
                    ...
                    added = await s.add_signal(sig)
                    if added:
                        s.recent_super_signals[symbol] = max_t
                        s.log("WS mid-candle: SUPER signal detected for", symbol, "components:", sig["components"])
            return
    except Exception as e:
        try:
...
async def root_scanner_loop(root_tf: str):
    ...
    while True:
        ...
        # Always sleep per configured interval and continue
        await asyncio.sleep(s.SCAN_INTERVAL_SECONDS)
        ...
        try:
            ...
            for sid in ids:
                try:
                    ...
                    if s.is_stablecoin_symbol(symbol) or s.symbol_too_new(symbol):
...
async def super_signal_scanner_loop_5m():
    import services as s
    s.log("Super scanner started (5m loop), DISCOVERY_CONCURRENCY=", s.DISCOVERY_CONCURRENCY)
    semaphore = asyncio.Semaphore(s.DISCOVERY_CONCURRENCY)
    while True:
        try:
            symbols = await s.get_tradable_usdt_symbols()
            ...
            async def _check_super(sym: str):
                async with semaphore:
                    try:
                        ...
                        for tf in ["1h", "4h", "1d"]:
                            try:
                                flip_kind, last_ts = await s.detect_flip(sym, tf)
                        ...
                        if len(flips) >= 2:
                            ...
                            if (max_t - min_t) <= s.SUPER_WINDOW_SECONDS:
                                # dedupe
                                last_super = s.recent_super_signals.get(sym)
                                if last_super and (max_t - last_super) <= s.SUPER_WINDOW_SECONDS:
                                    s.log("Skipped super creation (recent) for", sym)
                                    return
                                if s.signal_exists_for(sym, "super"):
                                    s.log("Skipped super creation (already active) for", sym)
                                    return
                                key = f"SUPER-{sym}-{max_t}"
                                sig = {
                                    "id": key,
                                    "symbol": sym,
                                    "root_tf": "multi",
                                ...
                                added = await s.add_signal(sig)
                                if added:
                                    s.recent_super_signals[sym] = max_t
                                    s.log("SUPER signal detected for", sym, "components:", sig["components"])
                    except Exception as e:
                        s.log("super check error for", sym, e)
            for s_sym in symbols:
                tasks.append(asyncio.create_task(_check_super(s_sym)))
            if tasks:
                await asyncio.gather(*tasks, return_exceptions=True)
        except Exception as e:
            s.log("super_signal_scanner_loop_5m error:", e)
        await asyncio.sleep(s.SCAN_INTERVAL_SECONDS)
            ...
            for sid, sig in list(s.active_root_signals.items()):
                try:
                    ...
                    if stype == "root":
                        ...
                        if flip_time and now_s >= (flip_time + s.SUPER_WINDOW_SECONDS):
                            to_remove.append(sid)
                except Exception:
                    continue
