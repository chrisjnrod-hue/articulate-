...
# ✅ MTF Monitoring:  Track TF state for each signal with prior state
...
# ✅ FULLY CORRECTED:  notify_alignment_if_ready with proper MTF state tracking
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    """
    Check MTF alignment. 
    
    BUG FIXES:
    1. ✅ Changed 'was_positive' to 'prior_positive' for clarity
    2. ✅ CRITICAL FIX: Update prior_positive AFTER checking conditions
    3. ✅ Transition detection works:  detects negative→positive flip
    4. ✅ Handles case where all TFs already positive on first check
    5. ✅ Persists prior state correctly for next invocation
    
    Logic:
    1. Root TF must show flip="open"
    2. Monitor all 5 TFs (5m, 15m, 1h, 4h, 1d)
    3. Track transitions from negative → positive
    4. If any TF negative:  log and wait
    5. When all positive: fire entry with last TF to flip info
    """
    try:  
        ...
        log(f"[MTF] ========== START notify_alignment_if_ready ==========")
        log(f"[MTF] symbol={symbol}, root_tf={root_tf}, stype={stype}, signal_id={signal_id}")

        if stype != "root" or not symbol or not root_tf:
            log(f"[MTF] SKIP: stype != root or missing symbol/root_tf")
            return
        ...
        log(f"[MTF] Required TFs: {required_tfs}")

        tf_status = {}

        # ============ STEP 1: Gather current TF status ============
        log(f"[MTF] [STEP 1] GATHERING TF STATUS")
        for tf in required_tfs:
            token = TF_MAP.get(tf)

            if not token:
                tf_status[tf] = {"has":  False, "positive": False}
                log(f"[MTF]   {tf}: NO TOKEN in TF_MAP → SKIP")
                continue
            ...
            if not dq or len(dq) < MIN_CANDLES_REQUIRED:  
                tf_status[tf] = {"has": False, "positive": False}
                log(f"[MTF]   {tf}: cache_missing (need {MIN_CANDLES_REQUIRED}, have {cache_size}) → SKIP")
                continue
            ...
            tf_status[tf] = {"has": True, "positive": positive, "macd_last": last_hist}
            log(f"[MTF]   {tf}: ✓ positive={positive}, macd_hist={last_hist:. 6f}" if last_hist else f"[MTF]   {tf}: ✓ positive={positive}")

        # ============ STEP 2: Validate root TF has open flip ============
        log(f"[MTF] [STEP 2] VALIDATING ROOT TF FLIP")
        ...
        log(f"[MTF]   detect_flip: kind={flip_kind}, ts={flip_ts}")

        if flip_kind != "open":
            log(f"[MTF] SKIP: Root TF flip not open (flip_kind={flip_kind})")
            return

        log(f"[MTF]   ✅ Root TF flip IS OPEN")

        # ============ STEP 3: Check for missing/negative TFs ============
        log(f"[MTF] [STEP 3] CHECKING FOR MISSING/NEGATIVE TFs")
        ...
        if missing_tfs: 
            log(f"[MTF]   ⚠ Missing:  {missing_tfs}")
        if negative_tfs:
            log(f"[MTF]   ⚠ Negative: {negative_tfs}")

        # ✅ If ANY TF is missing or negative, wait
        if missing_tfs or negative_tfs:
            log(f"[MTF] WAITING for alignment (missing or negative TFs)")
            
            # ============ STEP 4A: Update states for WAITING condition ============
            log(f"[MTF] [STEP 4A] UPDATING STATE (WAITING)")
            for tf in required_tfs:
                ...
                log(f"[MTF]   {tf}: prior={mtf_monitoring[signal_id][tf]['prior_positive']} → current={current_positive}")
            
            log(f"[MTF] ========== END notify_alignment_if_ready (WAITING) ==========")
            return

        # ============ STEP 4B: All positive!  Detect transitions & fire entry ============
        log(f"[MTF] [STEP 4B] MONITORING STATE TRANSITIONS (all positive! )")
        ...
        for tf in required_tfs: 
            ...
            # ✅ CRITICAL FIX: Check prior state, not "was_positive"
            # If this is the first time positive AND has data, it just flipped
            if current_positive and not prior_positive and has_data:
                last_flipped_tf = tf
                log(f"[MTF]   🔄 {tf}: TRANSITION DETECTED (negative→positive)")
                mtf_monitoring[signal_id][tf]["first_positive_time"] = int(time.time())
            else:
                log(f"[MTF]   ✓ {tf}: prior={prior_positive}, current={current_positive}")

            # ✅ CRITICAL FIX: Update AFTER checking transitions
            mtf_monitoring[signal_id][tf]["prior_positive"] = current_positive

        # ============ STEP 5: Fire entry signal ============
        log(f"[MTF] ✅✅✅ ALIGNMENT CONFIRMED ✅✅✅")
        log(f"[MTF] symbol={symbol}, root_tf={root_tf}, last_flipped_tf={last_flipped_tf or 'root'}")
        ...
        log(f"[MTF] add_signal returned: {added}")
        ...
        log(f"[MTF] ========== END notify_alignment_if_ready (ENTRY CREATED) ==========")
