...
# ✅ FIX: NEW STRONG IMPLEMENTATION - notify_alignment_if_ready
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    """
    Check if signal has reached alignment/entry conditions,
    update status, and send Telegram alert immediately if ready.
    """
    try:
        ...
        if signal_exists_for(symbol, "entry"):
            log(f"MTF check skipped for {symbol}:  already has entry signal")
            return

        required_tfs = tf_list_for_root(root_tf)
        tf_status = {}
        
        # Gather MTF status for all required timeframes
        for tf in required_tfs:
            token = TF_MAP[tf]
            dq = cache_get(symbol, token)
            
            if not dq or len(dq) < MIN_CANDLES_REQUIRED: 
                tf_status[tf] = {"has":  False, "reason": "insufficient_candles"}
                log(f"MTF check {symbol} {tf}: insufficient candles ({len(dq) if dq else 0} < {MIN_CANDLES_REQUIRED})")
                continue
            ...
            log(f"MTF check {symbol} {tf}: positive={positive} flip_last={flip_last}")

        # Validate root TF conditions
        root_status = tf_status.get(root_tf, {})
        if not root_status.get("has"):
            log(f"MTF alignment rejected for {symbol}: root TF {root_tf} missing data")
            return
        
        if not root_status.get("flip_last"):
            log(f"MTF alignment rejected for {symbol}: root TF {root_tf} no open flip")
            return

        # Check all required TFs have positive MACD
        missing_positive = []
        for tf in required_tfs:
            if not tf_status.get(tf, {}).get("positive"):
                missing_positive.append(tf)
        
        if missing_positive:  
            log(f"MTF alignment rejected for {symbol}: TFs {missing_positive} not positive")
            return

        # ✅ ALL CONDITIONS MET - UPDATE STATUS AND SEND ALERT
        log(f"🟢 MTF ALIGNMENT READY FOR {symbol} {root_tf}")
        ...
        try:
            ...
            await db.commit()
            log(f"DB updated:  signal {sig_id} status=entry")
        except Exception as e:
            log(f"DB update error for {sig_id}: {e}")
        ...
        for tf in required_tfs:
            st = tf_status.get(tf, {})
            if st.get("has"):
                pos_emoji = "✅" if st. get("positive") else "❌"
                flip_emoji = "📈" if st.get("flip_last") else "📉"
                alert_lines.append(f"  {tf}: {pos_emoji} MACD {flip_emoji} Flip")
            else:
        ...
        log(f"MTF alignment alert SENT for {symbol} {root_tf} ({sig_id})")
