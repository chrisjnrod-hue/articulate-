...
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    ...
    try: 
        ...
        if not root_token: 
            log(f"Root TF {root_tf} not in TF_MAP")
            return

        dq_root = cache_get(symbol, root_token)
        if not dq_root or len(dq_root) < MIN_CANDLES_REQUIRED:
            log(f"Root cache missing for {symbol} {root_tf}")
            return

        flip_kind, _ = await detect_flip(symbol, root_tf)
        if flip_kind != "open": 
            return
        ...
        for tf in required_tfs:
            ...
            if current_positive and not was_positive and tf_status.get(tf, {}).get("has"):
                last_flipped_tf = tf
                log(f"🔄 {symbol} {root_tf}:  {tf} just flipped positive!")
        ...
        if missing_tfs or negative_tfs:
            status_msg = f"Alignment waiting for {symbol} {root_tf}:"
            if missing_tfs:
        ...
        log(f"✅ ALIGNMENT MET for {symbol} {root_tf}:  ALL TFs positive, last to flip:  {last_flipped_tf or 'root'}")
        ...
        if added:
            ...
            try:
                await send_telegram(f"✅ MTF ENTRY:  {symbol} {root_tf} — All aligned{last_tf_msg}")
            except Exception: 
                log("Failed to send Telegram for MTF entry")
