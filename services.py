...
# ---------- Helper functions (instruments/candles) ----------
async def get_tradable_usdt_symbols() -> List[str]:
    ...
    for ep in INSTRUMENTS_ENDPOINTS:
        ...
        if isinstance(resp, dict):
            items = resp.get("result", {}).get("list", []) or resp.get("result", {}).get("data", []) or resp.get("data", []) or resp.get("result", []) or resp.get("symbols", []) or resp.get("rows", [])
...
# ---------- Notify MTF alignment helper ----------
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    """
    After a root is created, run a quick alignment check and send a Telegram alert
    if the multi-timeframe alignment / entry readiness is satisfied.
    """
    try:
        ...
        if stype == "root":
            ...
            # Fallback to root tf list for any non-root type (no other types are used)
            required_tfs = tf_list_for_root(root_tf)
        # ensure caches
        await asyncio.gather(*(ensure_cached_candles(symbol, tf, MIN_CANDLES_REQUIRED) for tf in required_tfs))
        # evaluate same logic as evaluate_signals_loop but non-destructive — only notify
        tf_status = {}
        for tf in required_tfs:
            ...
            tf_status[tf] = {"has": True, "closed": closed, "positive": positive, "flip_last": flip_last, "last_ts": list(dq)[-1]["start"]}
        # Root must be flipped
        if stype == "root":
        ...
        if len(negatives) == 1:
            ...
            if not st.get("has") or not st.get("flip_last"):
                return
            for tf in other_tfs:
                if tf == tf_to_flip:
                    continue
                if not tf_status.get(tf, {}).get("positive"):
                    return
        # At this point alignment/entry readiness is satisfied: notify
        comps = ", ".join(required_tfs)
        msg = f"MTF alignment confirmed for {symbol} (root={root_tf}) components={comps}"
        await send_telegram(msg)
        log("MTF alignment notification sent for", symbol, "root", root_tf)
    except Exception as e:
        log("notify_alignment_if_ready error:", e)
...
async def remove_signal(sig_id: str):
    ...
    # If we removed a root, unsubscribe 5m/15m for that symbol (they are only subscribed for active roots)
    if stype == "root":
        try:
            global public_ws
            if public_ws:
