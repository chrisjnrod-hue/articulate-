...
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    """
    ✅ CORRECT Multi-Timeframe (MTF) Logic: 
    - Root TF must have OPEN flip (mid-candle signal)
    - ALL other required TFs must be MACD positive
    - If root is true and ANY other TF is negative:  MONITOR (status='monitoring')
    - Once ALL are positive:  status='entry', send Telegram alert
    - Works regardless of which TF flips first (root can be first, last, or middle)
    """
    try:
        ...
        # Don't double alert if already entry
        if signal_exists_for(symbol, "entry"):
            log(f"[MTF] {symbol}:  already has entry signal, skipping")
            return

        # Define all possible TFs and required subordinates
        all_tfs = ["5m", "15m", "1h", "4h", "1d"]
        required_positives = [tf for tf in all_tfs if tf != root_tf]

        # Step 1: ROOT TF must have OPEN flip (mid-candle signal)
        root_token = TF_MAP.get(root_tf)
        root_dq = cache_get(symbol, root_token)
        
        if not root_dq or len(root_dq) < MIN_CANDLES_REQUIRED: 
            log(f"[MTF] {symbol} {root_tf}: ROOT insufficient candles ({len(root_dq) if root_dq else 0})")
            return
        
        root_flip_kind, _ = await detect_flip(symbol, root_tf)
        root_flip_open = (root_flip_kind == "open")
        
        if not root_flip_open: 
            log(f"[MTF] {symbol} {root_tf}: No open root flip yet")
            return

        log(f"[MTF_ROOT_FLIP] {symbol} {root_tf}: ROOT FLIP OPEN ✅")

        # Step 2: Check ALL subordinate TFs for MACD positive
        tf_status = {}
        monitoring_tfs = []

        for tf in required_positives:
            token = TF_MAP.get(tf)
            dq = cache_get(symbol, token)
            
            if not dq or len(dq) < MIN_CANDLES_REQUIRED:
                tf_status[tf] = {"status": "missing", "positive": None}
                monitoring_tfs.append(tf)
                log(f"[MTF] {symbol} {tf}: MISSING DATA")
                continue

            closes = candles_to_closes(dq)
            positive = macd_positive_from_closes(closes)
            flip_kind, _ = await detect_flip(symbol, tf)
            flip_open = (flip_kind == "open")
            ...
            if positive:
                log(f"[MTF] {symbol} {tf}: ✅ POSITIVE")
            else:
                monitoring_tfs.append(tf)
                log(f"[MTF] {symbol} {tf}:  ❌ NEGATIVE (waiting for flip)")

        # Step 3: Decide action based on monitoring status
        if monitoring_tfs:
            log(f"[MTF_MONITOR] {symbol} {root_tf}: Monitoring negative/missing TFs:  {monitoring_tfs}")

            # Keep in 'monitoring' state - will retry on next evaluation
            sig["status"] = "monitoring"
            sig["monitoring_for_tfs"] = monitoring_tfs
            active_root_signals[sig_id] = sig
            
            try:
                await db.execute(
                    "UPDATE root_signals SET status = ?  WHERE id = ?",
                    ("monitoring", sig_id),
                )
                await db.commit()
            except Exception as e:
                log(f"MTF DB update error: {e}")
            
            return  # Wait for next check

        # Step 4: ALL TFs are positive - SEND ALERT! 
        log(f"✅✅✅ [MTF_ENTRY_READY] {symbol} {root_tf}: All TFs now positive, sending alert!")
        
        sig["status"] = "entry"
        sig["mtf_alignment_time"] = now_ts_ms()
        active_root_signals[sig_id] = sig
        ...
        # Send alert with full status breakdown
        alert_lines = [
            "🟢🟢🟢 MTF ENTRY SIGNAL READY 🟢🟢🟢",
            f"Symbol: {symbol}",
            f"Root TF: {root_tf} (OPEN FLIP)",
            f"Signal ID: {sig_id}",
            "",
            "All TimeFrames Status:",
        ]
        
        alert_lines.append(f"  {root_tf} (ROOT): ✅ FLIP OPEN, MACD POSITIVE")
        
        for tf in required_positives:
            st = tf_status. get(tf, {})
            if st.get("status") == "positive":
                alert_lines.append(f"  {tf}: ✅ MACD POSITIVE")
            elif st.get("status") == "negative":
                alert_lines. append(f"  {tf}:  ❌ MACD NEGATIVE")
            else:
                alert_lines. append(f"  {tf}:  ⚠️ MISSING DATA")
        
        alert_lines.append("")
        alert_lines.append("🚀 Ready for ENTRY execution!")
        
        alert_msg = "\n".join(alert_lines)
        await send_telegram(alert_msg)
        
        log(f"✅ MTF alert SENT for {symbol} {root_tf} ({sig_id})")
...
# ✅ Call notify_alignment_if_ready IMMEDIATELY after adding signal
try:
    await notify_alignment_if_ready(sig)
except Exception as e: 
    log("notify_alignment_if_ready error after add:", e)

# Then subscribe/prewarm in background (non-blocking)
try:
    ...
    asyncio.create_task(_post_add_flow())  # ✅ Background only for subscriptions
except Exception: 
    pass
return True    
