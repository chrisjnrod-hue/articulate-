...
# ✅ FIX:  Implement notify_alignment_if_ready (create entry and send alert)
async def notify_alignment_if_ready(sig: Dict[str, Any]):
    ...
    try:
        ...
        # Quick check:  if already has entry, skip
        ...
        # Require root TF to be present and have an open flip
        if not tf_status.get(root_tf, {}).get("has"):
            return
        if not tf_status.get(root_tf, {}).get("flip_last"):
            return

        # Decide alignment: require all required TFs to have positive MACD (you can tune this)
        all_positive = all(tf_status.get(tf, {}).get("positive") for tf in required_tfs if tf_status.get(tf, {}).get("has"))
        if not all_positive:
            # Not yet aligned across TFs
            log(f"Alignment check for {symbol} {root_tf}: not all TFs positive yet", tf_status)
            return

        # Alignment met: create an 'entry' signal and notify
        log(f"Alignment check for {symbol} {root_tf}: conditions met", tf_status)
        ...
        if added:
            try:
                await send_telegram(f"MTF ENTRY: {symbol} {root_tf} — {entry_id}")
            except Exception:
